#!/usr/bin/env node
/**
 * Query Dawarich deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service dawarich query -- [--instance a]
...
import {
  listDawarichDeploymentSummaries,
  normalizeDawarichConfig,
  resolveDawarichDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/dawarich-install.mjs";
import { queryDawarichInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeDawarichConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listDawarichDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveDawarichDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryDawarichInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.dawarich,
