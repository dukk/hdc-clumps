 ...
 */
export async function installDawarichInCt(user, pveHost, vmid, dawarich, install, secrets) {
  errout.write(`[hdc] dawarich install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "dawarich install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const envContent = renderDawarichEnv(dawarich, secrets, ip);
  ...
  errout.write(`[hdc] dawarich install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(dawarich, ip),
    upstream_url: resolveUpstreamUrl(ip, dawarich),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainDawarichInCt(user, pveHost, vmid, dawarich, install, secrets, opts = {}) {
  errout.write(`[hdc] dawarich maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "dawarich maintain");
  if (!ready) {
