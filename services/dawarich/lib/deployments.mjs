...
import {
  ...
  storeGeodata,
  timeZone,
} from "./dawarich-render.mjs";

const DAWARICH_ROLE = "dawarich";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(DAWARICH_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.dawarich)) defaults.dawarich = structuredClone(cfg.dawarich);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeDawarichConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("dawarich config must be a JSON object");
  }
  ...
  throw new Error("dawarich config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(DAWARICH_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match dawarich-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const dawarich = isObject(d.dawarich) ? d.dawarich : {};
    const publicUrl = typeof dawarich.public_url === "string" ? dawarich.public_url.trim() : "";
    if (publicUrl) {
      parsePublicUrl(dawarich);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listDawarichDeploymentSummaries(cfg) {
  const { deployments } = normalizeDawarichConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(dawarich),
      postgis_image_tag: normalizePostgisImageTag(dawarich),
      redis_image_tag: normalizeRedisImageTag(dawarich),
      host_port: hostPort(dawarich),
      public_url: publicUrl,
      rails_env: railsEnv(dawarich),
      application_protocol: applicationProtocol(dawarich),
      time_zone: timeZone(dawarich),
      store_geodata: storeGeodata(dawarich),
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(DAWARICH_ROLE).test(t)) return t;
  return lxcSystemId(DAWARICH_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    dawarich: isObject(d.dawarich) ? d.dawarich : {},
    install,
  };
}
 ...
 */
export function resolveDawarichDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeDawarichConfig(cfg);
