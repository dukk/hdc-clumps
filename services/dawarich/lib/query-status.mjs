...
import { composeDir, hostPort, parsePublicUrl } from "./dawarich-render.mjs";
 ...
 */
export async function queryDawarichInCt(user, pveHost, vmid, dawarich, install) {
  ...
  if (docker.stdout.trim() === "active") {
    const healthCmd = `curl -sf --max-time 10 http://127.0.0.1:${port}/api/v1/health | grep -q '"status"' && echo ok || echo fail`;
