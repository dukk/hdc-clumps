#!/usr/bin/env node
/**
 * Deploy Dawarich on Proxmox LXC (Docker Compose: PostGIS + Redis + Sidekiq).
 *
 * Usage: hdc run service dawarich deploy -- [--instance a | --system-id dawarich-a] [--skip-install]
 *        hdc run service dawarich deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { installDawarichInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/dawarich-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/dawarich-render.mjs";
import {
...
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, dawarich, install } = deployment;
  ...
  if (shouldInstall(install)) {
    installResult = await installDawarichInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      dawarichCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(dawarichCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, dawarichCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(dawarichCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: dawarich LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveDawarichDeployments>} */
  let deployments;
  try {
