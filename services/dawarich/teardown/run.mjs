#!/usr/bin/env node
/**
 * Teardown dawarich Proxmox LXC deployments.
 *
 * Usage: hdc run service dawarich teardown -- [--instance a | --system-id dawarich-a]
 *        hdc run service dawarich teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/dawarich-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down dawarich LXC (stderr log; JSON on stdout).\n`);
