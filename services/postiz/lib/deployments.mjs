...
import {
  ...
  normalizePublicUrl,
  normalizeVersion,
} from "./postiz-render.mjs";

const POSTIZ_ROLE = "postiz";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(POSTIZ_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.postiz)) defaults.postiz = structuredClone(cfg.postiz);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizePostizConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("postiz config must be a JSON object");
  }
  ...
  throw new Error("postiz config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(POSTIZ_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match postiz-<letter>`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listPostizDeploymentSummaries(cfg) {
  const { deployments } = normalizePostizConfig(cfg);
  return deployments.map((d) => {
    ...
    const postiz = isObject(d.postiz) ? d.postiz : {};
    return {
      ...
      version: normalizeVersion(postiz),
      listen_port: listenPort(postiz),
      public_url: normalizePublicUrl(postiz.public_url),
      storage_provider:
        typeof postiz.storage_provider === "string" ? postiz.storage_provider : "local",
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(POSTIZ_ROLE).test(t)) return t;
  return lxcSystemId(POSTIZ_ROLE, t);
}

/**
 * @param {Record<string, unknown>} postiz
 */
export function secretVaultKeysFromConfig(postiz) {
  return {
    db: dbPasswordVaultKey(postiz),
    jwt: jwtSecretVaultKey(postiz),
  };
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    postiz: isObject(d.postiz) ? d.postiz : {},
    install,
  };
}
 ...
 */
export function resolvePostizDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizePostizConfig(cfg);
