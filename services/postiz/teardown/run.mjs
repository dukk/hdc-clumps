#!/usr/bin/env node
/**
 * Teardown Postiz Proxmox LXC deployments.
 *
 * Usage: hdc run service postiz teardown -- [--instance a] [--dry-run] [--yes]
 */
...
import { resolvePveSshForHost, stopPostizServicesInCt } from "hdc/package/postiz-install.mjs";
...
async function teardownOne(deployment, flags) {
  ...
  try {
    ...
    errout.write(`[hdc] ${target} ${verb}: stopping Postiz services in CT ${vmid} …\n`);
    stopPostizServicesInCt(pveSsh.user, pveSsh.host, vmid);
  } catch (e) {
    errout.write(
      `[hdc] ${target} ${verb}: stop services warning: ${String(/** @type {Error} */ (e).message || e)}\n`,
    );
  }
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Postiz LXC (stderr log; JSON on stdout).\n`);
