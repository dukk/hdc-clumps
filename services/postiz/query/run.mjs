#!/usr/bin/env node
/**
 * Query Postiz deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service postiz query -- [--instance a] [--live]
 */
...
import { resolvePveSshForHost } from "hdc/package/postiz-install.mjs";
import { queryPostizInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  const rel = relative(root, (_pkgConfig?.path ?? loaded.path ?? CLUMP_CONFIG_EXAMPLE)).replace(/\\/g, "/");
  ...
  if (cfg) {
    try {
      const norm = normalizePostizConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listPostizDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolvePostizDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryPostizInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.postiz,
