#!/usr/bin/env node
/**
 * Deploy Postiz on Proxmox LXC (native pnpm build + nginx).
 *
 * Usage: hdc run service postiz deploy -- [--instance a | --system-id postiz-a] [--skip-install]
 *        hdc run service postiz deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { installPostizInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/postiz-install.mjs";
import { normalizeVersion, resolveAccessUrl } from "hdc/package/postiz-render.mjs";
import { resolveReleaseTarget } from "hdc/package/postiz-release.mjs";
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; dbPassword: string; jwtSecret: string }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, postiz, install } = deployment;
  ...
  /** @type {{ ok: boolean; method?: string; message?: string; access_url?: string | null } | null} */
  ...
  if (shouldInstall(install)) {
    const version = normalizeVersion(postizCfg);
    errout.write(`[hdc] ${target} ${verb}: resolving release ${version} …\n`);
    const release = await resolveReleaseTarget(version);
    installResult = await installPostizInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      postizCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    access_url: installResult.access_url ?? resolveAccessUrl(postizCfg, ip),
    version: installResult.version ?? normalizeVersion(postizCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Postiz LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  const { dbPassword, jwtSecret } = await resolvePostizSecrets(vault, defaultsPz);
  ...
  for (const deployment of deployments) {
    try {
      results.push(await deployOne(deployment, flags, log, { ctPasswordCache, dbPassword, jwtSecret }));
    } catch (e) {
      const msg = String(/** @type {Error} */ (e).message || e);
