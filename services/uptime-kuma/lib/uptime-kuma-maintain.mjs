...
export function buildUpgradeScript(tag, tarballUrl) {
  ...
  return [
    ...
    "DATA_BAK=$(mktemp -d /tmp/uptime-kuma-data.XXXXXX)",
    "if [ -d /opt/uptime-kuma/data ]; then cp -a /opt/uptime-kuma/data \"$DATA_BAK/data\"; fi",
    ...
    "if [ -d \"$DATA_BAK/data\" ]; then rm -rf /opt/uptime-kuma/data; mv \"$DATA_BAK/data\" /opt/uptime-kuma/data; fi",
    "rm -rf \"$DATA_BAK\"",
    "cd /opt/uptime-kuma",
