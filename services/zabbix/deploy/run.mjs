#!/usr/bin/env node
/**
 * Deploy Zabbix on Proxmox LXC or QEMU (Docker Compose).
 *
 * Usage: hdc run service zabbix deploy -- [--instance a | --system-id vm-zabbix-a] [--skip-install]
 *        hdc run service zabbix deploy -- [--skip-existing | --redeploy-existing | --destroy-existing]
 */
...
} from "hdc/package/zabbix-install.mjs";
 ...
 * @param {Record<string, string>} flags
 * @param {{ dbPassword: string; dbRootPassword: string }} runOpts
 */
async function runConfigure(deployment, flags, runOpts) {
  const { systemId, mode, zabbix, install, configure } = deployment;
  ...
  return installZabbixOnHost(exec, zabbixCfg, installCfg, runOpts.dbPassword, runOpts.dbRootPassword);
}
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; dbPassword: string; dbRootPassword: string }} runOpts
 */
async function deployLxcOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, zabbix, install } = deployment;
  ...
  if (!skipProvision) {
    ...
    const hostname = (typeof lxc.hostname === "string" && lxc.hostname.trim()) || lxcHostnameFromSystemId(systemId) || "zabbix";
  ...
  const installResult = shouldInstall(install)
    ? await installZabbixInCt(
        pveSsh.user,
        pveSsh.host,
        guestVmid,
        isObject(zabbix) ? zabbix : {},
...
async function deployQemuOne(deployment, flags, log, runOpts) {
  const { systemId, proxmox: px, zabbix, install } = deployment;
  const inv = deployTargetInventory(root, target, { systemIdOverride: systemId });
  logDeployInventoryStatus(target, verb, inv);
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Zabbix via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  const vault = createZabbixVaultAccess();
  const { dbPassword, dbRootPassword } = await resolveZabbixDbSecrets(vault, defaultsZabbix);
  if (!dbPassword) {
  ...
  for (const deployment of deployments) {
    try {
      results.push(await deployOne(deployment, flags, log, { ctPasswordCache, dbPassword, dbRootPassword }));
    } catch (e) {
