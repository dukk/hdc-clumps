#!/usr/bin/env node
/**
 * Teardown Mosquitto Proxmox LXC deployments.
 *
 * Usage: hdc run service mosquitto teardown -- [--instance a | --system-id mosquitto-a]
 *        hdc run service mosquitto teardown -- [--dry-run] [--yes]
 */
...
import { resolveMosquittoDeployments } from "hdc/package/deployments.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Mosquitto LXC (stderr log; JSON on stdout).\n`);
