#!/usr/bin/env node
/**
 * Query Mosquitto deployments.
 *
 * Usage: hdc run service mosquitto query -- [--instance a]
...
import {
  listMosquittoDeploymentSummaries,
  normalizeMosquittoConfig,
  resolveMosquittoDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/mosquitto-install.mjs";
import { queryMosquittoInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeMosquittoConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listMosquittoDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveMosquittoDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryMosquittoInCt(pveSsh.user, pveSsh.host, vmid, d.mosquitto);
