#!/usr/bin/env node
/**
 * Deploy hdc-agents on Proxmox LXC (Docker Compose, local image build).
 *
 * Usage: hdc run service hdc-agents deploy -- [--instance a | --system-id hdc-agents-a] [--skip-install]
 *        hdc run service hdc-agents deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/hdc-agents-render.mjs";
import { syncHdcTreesViaPct } from "hdc/package/hdc-agents-sync.mjs";
...
async function deployOne(deployment, flags, log, runOpts) {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(ip, hdcAgentsCfg),
    upstream_url: resolveUpstreamUrl(ip, hdcAgentsCfg) ?? installResult.upstream_url ?? null,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: hdc-agents LXC via Proxmox (stderr log; JSON on stdout).\n`);
