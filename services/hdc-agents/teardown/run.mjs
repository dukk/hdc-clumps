#!/usr/bin/env node
/**
 * Teardown hdc-agents Proxmox LXC deployments.
 *
 * Usage: hdc run service hdc-agents teardown -- [--instance a | --system-id hdc-agents-a]
 *        hdc run service hdc-agents teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownHdcAgentsInCt, resolvePveSshForHost } from "hdc/package/hdc-agents-install.mjs";
...
async function teardownOne(deployment, flags) {
  ...
  if (!skipComposeDown) {
    try {
      ...
      await composeDownHdcAgentsInCt(pveSsh.user, pveSsh.host, vmid, installCfg);
    } catch (e) {
      errout.write(
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down hdc-agents LXC (stderr log; JSON on stdout).\n`);
