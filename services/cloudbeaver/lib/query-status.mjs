...
import { composeDir, hostPort, parsePublicUrl } from "./cloudbeaver-render.mjs";
 ...
 */
export async function queryCloudbeaverInCt(user, pveHost, vmid, cloudbeaver, install) {
  ...
  if (docker.stdout.trim() === "active") {
    const healthCmd = `curl -sf --max-time 10 http://127.0.0.1:${port}/ -o /dev/null && echo ok || echo fail`;
