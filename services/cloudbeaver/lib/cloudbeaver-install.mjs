 ...
 */
export async function installCloudbeaverInCt(user, pveHost, vmid, cloudbeaver, install, adminPassword) {
  errout.write(`[hdc] cloudbeaver install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "cloudbeaver install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const envContent = renderCloudbeaverEnv(cloudbeaver, ip, adminPassword);
  const composeYaml = renderComposeYaml(cloudbeaver);
  ...
  errout.write(`[hdc] cloudbeaver install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(cloudbeaver, ip),
    upstream_url: resolveUpstreamUrl(ip, cloudbeaver),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainCloudbeaverInCt(
  user,
  pveHost,
  vmid,
  cloudbeaver,
  ...
  errout.write(`[hdc] cloudbeaver maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "cloudbeaver maintain");
  if (!ready) {
