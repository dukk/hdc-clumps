...
const CLOUDBEAVER_ROLE = "cloudbeaver";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(CLOUDBEAVER_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.cloudbeaver)) defaults.cloudbeaver = structuredClone(cfg.cloudbeaver);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeCloudbeaverConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("cloudbeaver config must be a JSON object");
  }
  ...
  throw new Error("cloudbeaver config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(CLOUDBEAVER_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match cloudbeaver-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const cloudbeaver = isObject(d.cloudbeaver) ? d.cloudbeaver : {};
    const publicUrl = cloudbeaver.public_url;
    if (publicUrl !== null && publicUrl !== undefined) {
      const s = typeof publicUrl === "string" ? publicUrl.trim() : "";
      if (s) parsePublicUrl(cloudbeaver);
    }
    if (adminEnabled(cloudbeaver) && !adminPasswordVaultKey(cloudbeaver)) {
      throw new Error(`${sid}: cloudbeaver.admin.admin_password_vault_key required when admin enabled`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listCloudbeaverDeploymentSummaries(cfg) {
  const { deployments } = normalizeCloudbeaverConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image: normalizeImage(cloudbeaver),
      host_port: hostPort(cloudbeaver),
      public_url: publicUrl,
      admin_enabled: adminEnabled(cloudbeaver),
      admin_username: adminEnabled(cloudbeaver) ? adminUsernameFromCfg(cloudbeaver) : null,
    };
  });
}
 ...
 */
function adminUsernameFromCfg(cloudbeaver) {
  const admin = isObject(cloudbeaver.admin) ? cloudbeaver.admin : {};
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(CLOUDBEAVER_ROLE).test(t)) return t;
  return lxcSystemId(CLOUDBEAVER_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    cloudbeaver: isObject(d.cloudbeaver) ? d.cloudbeaver : {},
    install,
  };
}
 ...
 */
export function resolveCloudbeaverDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeCloudbeaverConfig(cfg);
