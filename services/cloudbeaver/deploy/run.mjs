#!/usr/bin/env node
/**
 * Deploy cloudbeaver on Proxmox LXC (Docker Compose).
 *
 * Usage: hdc run service cloudbeaver deploy -- [--instance a | --system-id cloudbeaver-a] [--skip-install]
 *        hdc run service cloudbeaver deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveCloudbeaverDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import {
  installCloudbeaverInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/cloudbeaver-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/cloudbeaver-render.mjs";
import { resolveCloudbeaverSecrets } from "hdc/package/vault-secrets.mjs";
import {
...
import { createCloudbeaverVaultAccess } from "hdc/package/vault-deps.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/cloudbeaver/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveCloudbeaverDeployments>[number]} deployment
 * @param {Record<string, string>} flags
...
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, cloudbeaver, install } = deployment;
  ...
  const cloudbeaverCfg = isObject(cloudbeaver) ? cloudbeaver : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    const secrets = await resolveCloudbeaverSecrets(runOpts.vault, cloudbeaverCfg);
    const adminPassword = secrets.adminPassword ?? "";
    installResult = await installCloudbeaverInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      cloudbeaverCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(cloudbeaverCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, cloudbeaverCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(cloudbeaverCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: cloudbeaver LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  let deployments;
  try {
    deployments = resolveCloudbeaverDeployments(cfg, flags);
  } catch (e) {
  ...
  const vault = createCloudbeaverVaultAccess();
  await vault.unlock({});
