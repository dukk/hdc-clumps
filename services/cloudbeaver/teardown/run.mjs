#!/usr/bin/env node
/**
 * Teardown cloudbeaver Proxmox LXC deployments.
 *
 * Usage: hdc run service cloudbeaver teardown -- [--instance a | --system-id cloudbeaver-a]
 *        hdc run service cloudbeaver teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/cloudbeaver-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down cloudbeaver LXC (stderr log; JSON on stdout).\n`);
