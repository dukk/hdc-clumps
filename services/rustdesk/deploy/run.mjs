#!/usr/bin/env node
/**
 * Deploy RustDesk OSS server on Proxmox LXC (Docker Compose, hbbs + hbbr).
 *
 * Usage: hdc run service rustdesk deploy -- [--instance a | --system-id rustdesk-a] [--skip-install]
 *        hdc run service rustdesk deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveRustdeskDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installRustdeskInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/rustdesk-install.mjs";
import { clientConfigSummary } from "hdc/package/rustdesk-render.mjs";
import {
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/rustdesk/config.example.json";
...
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, rustdesk, install } = deployment;
  ...
  /** @type {{ ok: boolean; method?: string; message?: string; id_server?: string | null; public_key?: string | null; client?: Record<string, unknown> } | null} */
  ...
  const rustdeskCfg = isObject(rustdesk) ? rustdesk : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installRustdeskInCt(pveSsh.user, pveSsh.host, guestVmid, rustdeskCfg, installCfg);
  } else {
  ...
  const client =
    installResult.client ?? clientConfigSummary(ip, installResult.public_key ?? null, rustdeskCfg);
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: RustDesk OSS LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveRustdeskDeployments>} */
  let deployments;
  try {
    deployments = resolveRustdeskDeployments(cfg, flags);
  } catch (e) {
