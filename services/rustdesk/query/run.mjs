#!/usr/bin/env node
/**
 * Query RustDesk deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service rustdesk query -- [--instance a]
...
import {
  listRustdeskDeploymentSummaries,
  normalizeRustdeskConfig,
  resolveRustdeskDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/rustdesk-install.mjs";
import { queryRustdeskInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeRustdeskConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listRustdeskDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveRustdeskDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryRustdeskInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.rustdesk,
