#!/usr/bin/env node
/**
 * Teardown RustDesk Proxmox LXC deployments.
 *
 * Usage: hdc run service rustdesk teardown -- [--instance a | --system-id rustdesk-a]
 *        hdc run service rustdesk teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/rustdesk-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down RustDesk LXC (stderr log; JSON on stdout).\n`);
