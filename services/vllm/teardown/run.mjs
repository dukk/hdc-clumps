#!/usr/bin/env node
/**
 * Teardown vLLM Proxmox QEMU deployments.
 *
 * Usage: hdc run service vllm teardown -- [--instance a | --system-id vm-vllm-a]
 *        hdc run service vllm teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownViaSsh } from "hdc/package/vllm-install.mjs";
  ...
  if (!skipComposeDown) {
    try {
      ...
      if (sshHost) {
        ...
        composeDownViaSsh({ exec, install: isObject(install) ? install : {} });
      }
    } catch (e) {
      errout.write(
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down vLLM QEMU (stderr log; JSON on stdout).\n`);
