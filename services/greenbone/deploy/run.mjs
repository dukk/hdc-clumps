...
import { installGreenboneInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/greenbone-install.mjs";
import { adminPasswordVaultKey, hostPort } from "hdc/package/greenbone-render.mjs";
...
async function deployOne(deployment, flags, log, runOpts) {
  ...
  if (!installResult.ok) {
    return { ok: false, system_id: systemId, host_id: hostId, mode, redeploy: skipProvision, result: provisionResult, install: installResult };
  }
