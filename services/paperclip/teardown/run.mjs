#!/usr/bin/env node
/**
 * Teardown paperclip Proxmox LXC deployments.
 *
 * Usage: hdc run service paperclip teardown -- [--instance a | --system-id paperclip-a]
 *        hdc run service paperclip teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/paperclip-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down paperclip LXC (stderr log; JSON on stdout).\n`);
