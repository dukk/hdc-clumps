 ...
 */
export async function installPaperclipInCt(user, pveHost, vmid, paperclip, install, secrets) {
  errout.write(`[hdc] paperclip install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "paperclip install");
  if (!ready) {
  ...
  errout.write(`[hdc] paperclip install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
...
export async function maintainPaperclipInCt(user, pveHost, vmid, paperclip, install, secrets, opts = {}) {
  ...
  const ready = await waitForCt(user, pveHost, vmid, 2000, "paperclip maintain");
  if (!ready) {
