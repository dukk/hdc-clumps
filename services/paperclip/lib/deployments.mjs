...
const PAPERCLIP_ROLE = "paperclip";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(PAPERCLIP_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.paperclip)) defaults.paperclip = structuredClone(cfg.paperclip);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizePaperclipConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("paperclip config must be a JSON object");
  }
  ...
  throw new Error("paperclip config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(PAPERCLIP_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match paperclip-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const paperclip = isObject(d.paperclip) ? d.paperclip : {};
    const publicUrl = typeof paperclip.public_url === "string" ? paperclip.public_url.trim() : "";
    if (publicUrl) {
      parsePublicUrl(paperclip);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listPaperclipDeploymentSummaries(cfg) {
  const { deployments } = normalizePaperclipConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(paperclip),
      postgres_image_tag: normalizePostgresImageTag(paperclip),
      host_port: hostPort(paperclip),
      public_url: publicUrl,
      deployment_mode: deploymentMode(paperclip),
      deployment_exposure: deploymentExposure(paperclip),
      telemetry_disabled: telemetryDisabled(paperclip),
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(PAPERCLIP_ROLE).test(t)) return t;
  return lxcSystemId(PAPERCLIP_ROLE, t);
}

/**
 * @param {Record<string, unknown>} paperclip
 */
export function betterAuthSecretVaultKeyFromConfig(paperclip) {
  return betterAuthSecretVaultKey(isObject(paperclip) ? paperclip : {});
}

/**
 * @param {Record<string, unknown>} paperclip
 */
export function dbPasswordVaultKeyFromConfig(paperclip) {
  return dbPasswordVaultKey(isObject(paperclip) ? paperclip : {});
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    paperclip: isObject(d.paperclip) ? d.paperclip : {},
    install,
  };
}
 ...
 */
export function resolvePaperclipDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizePaperclipConfig(cfg);
