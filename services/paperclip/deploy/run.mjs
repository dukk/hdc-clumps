#!/usr/bin/env node
/**
 * Deploy paperclip on Proxmox LXC (Docker Compose + PostgreSQL).
 *
 * Usage: hdc run service paperclip deploy -- [--instance a | --system-id paperclip-a] [--skip-install]
 *        hdc run service paperclip deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { installPaperclipInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/paperclip-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/paperclip-render.mjs";
import {
...
import { resolvePaperclipSecrets } from "hdc/package/vault-secrets.mjs";
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; secrets: { betterAuthSecret: string; dbPassword: string } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, paperclip, install } = deployment;
  ...
  if (shouldInstall(install)) {
    installResult = await installPaperclipInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      paperclipCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(paperclipCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, paperclipCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(paperclipCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: paperclip LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolvePaperclipDeployments>} */
  let deployments;
  try {
