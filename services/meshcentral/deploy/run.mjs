#!/usr/bin/env node
/**
 * Deploy MeshCentral on Proxmox LXC (Docker Compose + MongoDB).
 *
 * Usage: hdc run service meshcentral deploy -- [--instance a | --system-id meshcentral-a] [--skip-install]
 *        hdc run service meshcentral deploy -- [--skip-existing | --redeploy-existing]
 */
...
import {
  installMeshcentralInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/meshcentral-install.mjs";
import { serviceSummary } from "hdc/package/meshcentral-render.mjs";
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; vault: import("../../../lib/package-vault-access.mjs").PackageVaultAccess }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, meshcentral, install } = deployment;
  ...
  if (shouldInstall(install)) {
    try {
      const secrets = await resolveMeshcentralSecrets(runOpts.vault, meshcentralCfg);
  ...
  if (shouldInstall(install)) {
    installResult = await installMeshcentralInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      meshcentralCfg,
  ...
  const service =
    installResult.service ?? serviceSummary(ip, meshcentralCfg);
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: MeshCentral LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveMeshcentralDeployments>} */
  let deployments;
  try {
