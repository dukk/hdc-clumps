#!/usr/bin/env node
/**
 * Teardown MeshCentral Proxmox LXC deployments.
 *
 * Usage: hdc run service meshcentral teardown -- [--instance a | --system-id meshcentral-a]
 *        hdc run service meshcentral teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/meshcentral-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down MeshCentral LXC (stderr log; JSON on stdout).\n`);
