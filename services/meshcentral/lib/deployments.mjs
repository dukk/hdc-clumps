...
import { normalizeImageTag, resolveHostname, resolvePublicUrl } from "./meshcentral-render.mjs";

const MESHCENTRAL_ROLE = "meshcentral";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(MESHCENTRAL_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.meshcentral)) defaults.meshcentral = structuredClone(cfg.meshcentral);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeMeshcentralConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("meshcentral config must be a JSON object");
  }
  ...
  throw new Error("meshcentral config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(MESHCENTRAL_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match meshcentral-<letter>`);
    }
    ...
    const mc = isObject(d.meshcentral) ? d.meshcentral : {};
    try {
      if (!resolveHostname(mc) && !resolvePublicUrl(mc)) {
        throw new Error(`${sid}: meshcentral.hostname or meshcentral.public_url required`);
      }
    } catch (e) {
      throw new Error(`${sid}: ${/** @type {Error} */ (e).message || e}`);
    }
  }
}
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listMeshcentralDeploymentSummaries(cfg) {
  const { deployments } = normalizeMeshcentralConfig(cfg);
  return deployments.map((d) => {
    ...
    const meshcentral = isObject(d.meshcentral) ? d.meshcentral : {};
    let hostname = null;
    let publicUrl = null;
    try {
      hostname = resolveHostname(meshcentral);
      publicUrl = resolvePublicUrl(meshcentral);
    } catch {
      /* summary only */
    }
    return {
      ...
      image_tag: normalizeImageTag(meshcentral),
      hostname,
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(MESHCENTRAL_ROLE).test(t)) return t;
  return lxcSystemId(MESHCENTRAL_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    meshcentral: isObject(d.meshcentral) ? d.meshcentral : {},
    install,
  };
}
 ...
 */
export function resolveMeshcentralDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeMeshcentralConfig(cfg);
