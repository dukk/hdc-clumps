#!/usr/bin/env node
/**
 * Query Open WebUI deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service open-webui query -- [--instance a]
...
import { resolvePveSshForHost } from "hdc/package/open-webui-install.mjs";
import { queryOpenWebuiInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeOpenWebuiConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listOpenWebuiDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveOpenWebuiDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryOpenWebuiInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.openWebui,
