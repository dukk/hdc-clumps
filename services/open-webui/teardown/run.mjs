#!/usr/bin/env node
/**
 * Teardown Open WebUI Proxmox LXC deployments.
 *
 * Usage: hdc run service open-webui teardown -- [--instance a | --system-id open-webui-a]
 *        hdc run service open-webui teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { resolveOpenWebuiDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/open-webui-install.mjs";
import {
  confirmTeardown,
  teardownDryRun,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Open WebUI LXC (stderr log; JSON on stdout).\n`);
