...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(OPEN_WEBUI_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.open_webui)) defaults.open_webui = structuredClone(cfg.open_webui);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeOpenWebuiConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("open-webui config must be a JSON object");
  }
  ...
  throw new Error("open-webui config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(OPEN_WEBUI_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match open-webui-<letter>`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listOpenWebuiDeploymentSummaries(cfg) {
  const { deployments } = normalizeOpenWebuiConfig(cfg);
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(OPEN_WEBUI_ROLE).test(t)) return t;
  return lxcSystemId(OPEN_WEBUI_ROLE, t);
}

/**
 * @param {Record<string, unknown>} openWebui
 */
export function secretKeyVaultKeyFromConfig(openWebui) {
  const key =
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    openWebui: isObject(d.open_webui) ? d.open_webui : {},
    install,
  };
}
 ...
 */
export function resolveOpenWebuiDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeOpenWebuiConfig(cfg);
