...
import {
  installOpenWebuiInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/open-webui-install.mjs";
import { hostPort, normalizeOpenaiBackends, resolveWebUiUrl } from "hdc/package/open-webui-render.mjs";
...
import { waitForOpenWebuiProvisionTask } from "hdc/package/proxmox-task-wait.mjs";
...
async function deployOne(deployment, flags, log, runOpts) {
  ...
  if (!skipProvision) {
    ...
    await waitForOpenWebuiProvisionTask(
      provisionResult,
      auth,
  ...
  const uiUrl = resolveWebUiUrl(ip, openWebuiCfg) ?? installResult.web_ui_url ?? null;
  ...
  return {
    ...
    ip,
    web_ui_url: uiUrl,
    host_port: hostPort(openWebuiCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Open WebUI LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveOpenWebuiDeployments>} */
  let deployments;
  try {
  ...
  errout.write(`[hdc] ${target} ${verb}: loading WEBUI secret from vault ${skKey} …\n`);
  await vault.unlock({});
