#!/usr/bin/env node
/**
 * Teardown IT-Tools Proxmox LXC deployments.
 *
 * Usage: hdc run service it-tools teardown -- [--instance a | --system-id it-tools-a]
 *        hdc run service it-tools teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/it-tools-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down it-tools LXC (stderr log; JSON on stdout).\n`);
