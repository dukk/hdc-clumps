#!/usr/bin/env node
/**
 * Deploy IT-Tools on Proxmox LXC (Docker Compose, stateless).
 *
 * Usage: hdc run service it-tools deploy -- [--instance a | --system-id it-tools-a] [--skip-install]
 *        hdc run service it-tools deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveItToolsDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installItToolsInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/it-tools-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/it-tools-render.mjs";
import {
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/it-tools/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveItToolsDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, it_tools, install } = deployment;
  ...
  const itToolsCfg = isObject(it_tools) ? it_tools : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installItToolsInCt(pveSsh.user, pveSsh.host, guestVmid, itToolsCfg, installCfg);
  } else {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(itToolsCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, itToolsCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(itToolsCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: it-tools LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveItToolsDeployments>} */
  let deployments;
  try {
    deployments = resolveItToolsDeployments(cfg, flags);
  } catch (e) {
