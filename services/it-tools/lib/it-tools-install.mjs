 ...
 */
export async function installItToolsInCt(user, pveHost, vmid, itTools, install) {
  errout.write(`[hdc] it-tools install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "it-tools install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const composeYaml = renderComposeYaml(itTools, install);
  ...
  errout.write(`[hdc] it-tools install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(itTools, ip),
    upstream_url: resolveUpstreamUrl(ip, itTools),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainItToolsInCt(user, pveHost, vmid, itTools, install, opts = {}) {
  errout.write(`[hdc] it-tools maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "it-tools maintain");
  if (!ready) {
