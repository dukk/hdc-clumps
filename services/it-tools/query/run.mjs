#!/usr/bin/env node
/**
 * Query IT-Tools deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service it-tools query -- [--instance a]
...
import {
  listItToolsDeploymentSummaries,
  normalizeItToolsConfig,
  resolveItToolsDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/it-tools-install.mjs";
import { queryItToolsInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeItToolsConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listItToolsDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveItToolsDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryItToolsInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.it_tools,
