#!/usr/bin/env node
/**
 * Query omni-tools deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service omni-tools query -- [--instance a]
 *        hdc run service omni-tools query -- --live
 */
...
import {
  listOmniToolsDeploymentSummaries,
  normalizeOmniToolsConfig,
  resolveOmniToolsDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/omni-tools-install.mjs";
import { queryOmniToolsInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeOmniToolsConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listOmniToolsDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveOmniToolsDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryOmniToolsInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.omni_tools,
