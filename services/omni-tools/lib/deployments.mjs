...
import { hostPort, normalizeImage, parsePublicUrl } from "./omni-tools-render.mjs";

const OMNI_TOOLS_ROLE = "omni-tools";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(OMNI_TOOLS_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.omni_tools)) defaults.omni_tools = structuredClone(cfg.omni_tools);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeOmniToolsConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("omni-tools config must be a JSON object");
  }
  ...
  throw new Error("omni-tools config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(OMNI_TOOLS_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match omni-tools-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const OmniTools = isObject(d.omni_tools) ? d.omni_tools : {};
    const publicUrl = OmniTools.public_url;
    if (publicUrl !== null && publicUrl !== undefined) {
      const s = typeof publicUrl === "string" ? publicUrl.trim() : "";
      if (s) parsePublicUrl(OmniTools);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listOmniToolsDeploymentSummaries(cfg) {
  const { deployments } = normalizeOmniToolsConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image: normalizeImage(OmniTools),
      host_port: hostPort(OmniTools),
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(OMNI_TOOLS_ROLE).test(t)) return t;
  return lxcSystemId(OMNI_TOOLS_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    omni_tools: isObject(d.omni_tools) ? d.omni_tools : {},
    install,
  };
}
 ...
 */
export function resolveOmniToolsDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeOmniToolsConfig(cfg);
