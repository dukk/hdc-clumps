#!/usr/bin/env node
/**
 * Deploy omni-tools on Proxmox LXC (Docker Compose, stateless).
 *
 * Usage: hdc run service omni-tools deploy -- [--instance a | --system-id omni-tools-a] [--skip-install]
 *        hdc run service omni-tools deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveOmniToolsDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installOmniToolsInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/omni-tools-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/omni-tools-render.mjs";
import {
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/omni-tools/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveOmniToolsDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, omni_tools, install } = deployment;
  ...
  const OmniToolsCfg = isObject(omni_tools) ? omni_tools : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installOmniToolsInCt(pveSsh.user, pveSsh.host, guestVmid, OmniToolsCfg, installCfg);
  } else {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(OmniToolsCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, OmniToolsCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(OmniToolsCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: omni-tools LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveOmniToolsDeployments>} */
  let deployments;
  try {
    deployments = resolveOmniToolsDeployments(cfg, flags);
  } catch (e) {
