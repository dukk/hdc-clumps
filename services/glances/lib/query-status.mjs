...
import { composeDir, hostPort, parsePublicUrl } from "./glances-render.mjs";
 ...
 */
export async function queryGlancesInCt(user, pveHost, vmid, glances, install) {
  const cfg = isObject(glances) ? glances : {};
  ...
  if (docker.stdout.trim() === "active") {
    const healthCmd = `curl -sf --max-time 5 http://127.0.0.1:${port}/api/4/quicklook -o /dev/null && echo ok || echo fail`;
