#!/usr/bin/env node
/**
 * Query glances deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service glances query -- [--instance a]
 *        hdc run service glances query -- --live
 */
...
import {
  listGlancesDeploymentSummaries,
  normalizeGlancesConfig,
  resolveGlancesDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/glances-install.mjs";
import { queryGlancesInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeGlancesConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listGlancesDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveGlancesDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryGlancesInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.glances,
