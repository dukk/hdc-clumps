#!/usr/bin/env node
/**
 * Teardown glances Proxmox LXC deployments.
 *
 * Usage: hdc run service glances teardown -- [--instance a | --system-id glances-a]
 *        hdc run service glances teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/glances-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down glances LXC (stderr log; JSON on stdout).\n`);
