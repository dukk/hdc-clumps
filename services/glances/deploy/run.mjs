#!/usr/bin/env node
/**
 * Deploy glances on Proxmox LXC (Docker Compose, stateless).
 *
 * Usage: hdc run service glances deploy -- [--instance a | --system-id glances-a] [--skip-install]
 *        hdc run service glances deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveGlancesDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installGlancesInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/glances-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/glances-render.mjs";
import {
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/glances/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveGlancesDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, glances, install } = deployment;
  ...
  const glancesCfg = isObject(glances) ? glances : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installGlancesInCt(pveSsh.user, pveSsh.host, guestVmid, glancesCfg, installCfg);
  } else {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(glancesCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, glancesCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(glancesCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: glances LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveGlancesDeployments>} */
  let deployments;
  try {
    deployments = resolveGlancesDeployments(cfg, flags);
  } catch (e) {
