#!/usr/bin/env node
/**
 * Query vikunja deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service vikunja query -- [--instance a]
...
import { resolvePveSshForHost } from "hdc/package/vikunja-install.mjs";
import { queryVikunjaInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeVikunjaConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listVikunjaDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveVikunjaDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryVikunjaInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.vikunja,
