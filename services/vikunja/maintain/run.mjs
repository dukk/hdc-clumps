...
/**
 * Maintain vikunja: re-push compose + .env from config, refresh Docker images, guest baseline.
 *
 * Usage: hdc run service vikunja maintain -- [--instance a | --system-id vikunja-a]
 *        hdc run service vikunja maintain -- [--skip-upgrade] [--skip-clamav]
 */
...
import { resolveVikunjaDeployments } from "hdc/package/deployments.mjs";
import { maintainVikunjaInCt, resolvePveSshForHost } from "hdc/package/vikunja-install.mjs";
import { createVikunjaVaultAccess } from "hdc/package/vikunja-vault-deps.mjs";
import { resolveVikunjaSecrets } from "hdc/package/vault-secrets.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/vikunja/config.example.json";
...
async function maintainOne(deployment, flags, secrets, vaultAccess) {
  const { systemId, proxmox: px, vikunja, install } = deployment;
  ...
  const result = await maintainVikunjaInCt(
    pveSsh.user,
    pveSsh.host,
    vmid,
    vikunjaCfg,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: refresh vikunja Docker stack (stderr log; JSON on stdout).\n`);
