 ...
 */
export async function installVikunjaInCt(user, pveHost, vmid, vikunja, install, secrets) {
  errout.write(`[hdc] vikunja install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "vikunja install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const envContent = renderVikunjaEnv(vikunja, secrets, ip);
  ...
  errout.write(`[hdc] vikunja install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(vikunja, ip),
    upstream_url: resolveUpstreamUrl(ip, vikunja),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainVikunjaInCt(user, pveHost, vmid, vikunja, install, secrets, opts = {}) {
  errout.write(`[hdc] vikunja maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "vikunja maintain");
  if (!ready) {
