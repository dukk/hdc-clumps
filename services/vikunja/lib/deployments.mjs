...
import {
  ...
  parsePublicUrl,
} from "./vikunja-render.mjs";

const VIKUNJA_ROLE = "vikunja";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(VIKUNJA_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.vikunja)) defaults.vikunja = structuredClone(cfg.vikunja);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeVikunjaConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("vikunja config must be a JSON object");
  }
  ...
  throw new Error("vikunja config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(VIKUNJA_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match vikunja-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const vikunja = isObject(d.vikunja) ? d.vikunja : {};
    const publicUrl = typeof vikunja.public_url === "string" ? vikunja.public_url.trim() : "";
    if (publicUrl) {
      parsePublicUrl(vikunja);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listVikunjaDeploymentSummaries(cfg) {
  const { deployments } = normalizeVikunjaConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(vikunja),
      postgres_image_tag: normalizePostgresImageTag(vikunja),
      host_port: hostPort(vikunja),
      public_url: publicUrl,
      timezone: typeof vikunja.timezone === "string" ? vikunja.timezone : null,
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(VIKUNJA_ROLE).test(t)) return t;
  return lxcSystemId(VIKUNJA_ROLE, t);
}

/**
 * @param {Record<string, unknown>} vikunja
 */
export function jwtSecretVaultKeyFromConfig(vikunja) {
  return jwtSecretVaultKey(isObject(vikunja) ? vikunja : {});
}

/**
 * @param {Record<string, unknown>} vikunja
 */
export function dbPasswordVaultKeyFromConfig(vikunja) {
  return dbPasswordVaultKey(isObject(vikunja) ? vikunja : {});
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    vikunja: isObject(d.vikunja) ? d.vikunja : {},
    install,
  };
}
 ...
 */
export function resolveVikunjaDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeVikunjaConfig(cfg);
