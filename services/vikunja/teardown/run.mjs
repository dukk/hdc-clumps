#!/usr/bin/env node
/**
 * Teardown vikunja Proxmox LXC deployments.
 *
 * Usage: hdc run service vikunja teardown -- [--instance a | --system-id vikunja-a]
 *        hdc run service vikunja teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/vikunja-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down vikunja LXC (stderr log; JSON on stdout).\n`);
