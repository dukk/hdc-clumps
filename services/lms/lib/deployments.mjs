...
import { normalizeModelNames } from "./lms-models.mjs";

const LMS_ROLE = "lms";
const LMS_QEMU_SYSTEM_ID = /^vm-lms-[a-z]+$/;
 ...
 */
export function normalizeLmsConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("lms config must be a JSON object");
  }
  const version = typeof cfg.schema_version === "number" ? cfg.schema_version : 1;
  if (!Array.isArray(cfg.deployments) || cfg.deployments.length === 0) {
    throw new Error("lms config needs deployments[] with at least one entry");
  }
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!LMS_QEMU_SYSTEM_ID.test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match vm-lms-<letter> for proxmox-qemu`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listLmsDeploymentSummaries(cfg) {
  const { deployments } = normalizeLmsConfig(cfg);
  return deployments.map((d) => {
    ...
    const models = normalizeModelNames(d.lms);
    const lmsBlock = isObject(d.lms) ? d.lms : {};
    ...
    return {
      ...
      server_port:
        typeof server.port === "number" && Number.isFinite(server.port) ? server.port : 1234,
    };
  });
}
...
export function instanceFlagToSystemId(instance, deployments) {
  ...
  if (LMS_QEMU_SYSTEM_ID.test(t)) return t;
 ...
 */
export function resolveLmsDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeLmsConfig(cfg);
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  const install = isObject(d.install) ? { ...d.install } : { enabled: true, linux_user: "lms" };
  ...
  const loadOnStart =
    typeof lmsBlock.load_on_start === "string" && lmsBlock.load_on_start.trim()
      ...
      : null;
  const models = normalizeModelNames(lmsBlock);
  return {
