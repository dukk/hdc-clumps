#!/usr/bin/env node
/**
 * Teardown twenty Proxmox LXC or QEMU deployments.
 *
 * Usage: hdc run service twenty teardown -- [--instance a | --system-id vm-twenty-a]
 *        hdc run service twenty teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down twenty (stderr log; JSON on stdout).\n`);
