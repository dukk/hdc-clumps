...
import { readTwentyGuestPowerState } from "hdc/package/proxmox-guest-ensure.mjs";
import { resolvePveSshForHost } from "hdc/package/twenty-install.mjs";
import { resolveGuestSshUser } from "hdc/package/guest-ssh-resolve.mjs";
import { createConfigureExec } from "../../postfix-relay/lib/postfix-relay-configure.mjs";
import { queryTwentyInCt, queryTwentyOnGuest } from "hdc/package/query-status.mjs";
    ...
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          try {
            const power = await readTwentyGuestPowerState({ deployment: d, proxmoxRoot });
            failure.guest_power_state = power.guest_power_state;
            failure.guest_node = power.node;
            failure.guest_vmid = power.vmid;
          } catch {
            // Proxmox diagnostics are best-effort when live query fails.
          }
          liveResults.push(failure);
        }
      }
    }
  }
