...
import { resolveTwentyDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { ensureTwentyGuestReachable } from "hdc/package/proxmox-guest-ensure.mjs";
...
} from "hdc/package/twenty-install.mjs";
import {
  applyQemuCloudInit,
  cloneQemuGuest,
  locateGuest,
...
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/twenty-render.mjs";
import {
...
import { createTwentyVaultAccess } from "hdc/package/twenty-vault-deps.mjs";
import { resolveTwentySecrets } from "hdc/package/vault-secrets.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/twenty/config.example.json";
...
async function deployQemuOne(deployment, flags, log, secrets) {
  ...
  if (located) {
    ...
    if (action === "skip") {
      ...
      const guestEnsure = await ensureTwentyGuestReachable({
        deployment,
        proxmoxRoot,
        flags,
        waitForSsh: false,
      ...
      return {
        ...
        guest_power_state: guestEnsure.guest_power_state,
        guest_started: guestEnsure.guest_started,
      };
    }
...
async function main() {
  ...
  let secrets;
  try {
    secrets = await resolveTwentySecrets(vault, defaultsTwenty, { allowGenerate: true });
  } catch (e) {
    const msg = String(/** @type {Error} */ (e).message || e);
