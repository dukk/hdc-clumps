#!/usr/bin/env node
/**
 * Teardown netboot.xyz Proxmox LXC deployments.
 *
 * Usage: hdc run service netboot-xyz teardown -- [--instance a | --system-id netboot-xyz-a]
 *        hdc run service netboot-xyz teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/netboot-xyz-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down netboot.xyz LXC (stderr log; JSON on stdout).\n`);
