#!/usr/bin/env node
/**
 * Deploy netboot.xyz on Proxmox LXC (Docker Compose).
 *
 * Usage: hdc run service netboot-xyz deploy -- [--instance a | --system-id netboot-xyz-a] [--skip-install]
 *        hdc run service netboot-xyz deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveNetbootXyzDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import {
  installNetbootXyzInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/netboot-xyz-install.mjs";
import { resolveWebUiUrl } from "hdc/package/netboot-xyz-render.mjs";
import {
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/netboot-xyz/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveNetbootXyzDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, netboot_xyz: netbootXyz, install } = deployment;
  ...
  /** @type {{ ok: boolean; method?: string; message?: string; web_ui_url?: string | null } | null} */
  ...
  const netbootXyzCfg = isObject(netbootXyz) ? netbootXyz : {};
  ...
  if (shouldInstall(install)) {
    installResult = await installNetbootXyzInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      netbootXyzCfg,
  ...
  const uiUrl = resolveWebUiUrl(ip, netbootXyzCfg) ?? installResult.web_ui_url ?? null;
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: netboot.xyz LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveNetbootXyzDeployments>} */
  let deployments;
  try {
    deployments = resolveNetbootXyzDeployments(cfg, flags);
  } catch (e) {
