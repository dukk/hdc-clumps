#!/usr/bin/env node
/**
 * Query netboot.xyz deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service netboot-xyz query -- [--instance a]
...
import {
  listNetbootXyzDeploymentSummaries,
  normalizeNetbootXyzConfig,
  resolveNetbootXyzDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/netboot-xyz-install.mjs";
import { queryNetbootXyzInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeNetbootXyzConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listNetbootXyzDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveNetbootXyzDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryNetbootXyzInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.netboot_xyz,
