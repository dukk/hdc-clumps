...
const NETBOOT_XYZ_ROLE = "netboot-xyz";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(NETBOOT_XYZ_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.netboot_xyz)) defaults.netboot_xyz = structuredClone(cfg.netboot_xyz);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeNetbootXyzConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("netboot-xyz config must be a JSON object");
  }
  ...
  throw new Error("netboot-xyz config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(NETBOOT_XYZ_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match netboot-xyz-<letter>`);
    }
 ...
 */
export function webAppPort(netbootXyz) {
  const p =
    typeof netbootXyz.web_app_port === "number"
      ? netbootXyz.web_app_port
      : Number(netbootXyz.web_app_port);
 ...
 * @param {Record<string, unknown>} netbootXyz
 */
export function nginxHostPort(netbootXyz) {
  const p =
    typeof netbootXyz.nginx_host_port === "number"
      ? netbootXyz.nginx_host_port
      : Number(netbootXyz.nginx_host_port);
  if (Number.isFinite(p) && p >= 1 && p <= 65535) return Math.floor(p);
  return 8080;
}
 ...
 * @param {Record<string, unknown>} netbootXyz
 */
export function tftpHostPort(netbootXyz) {
  const p =
    typeof netbootXyz.tftp_host_port === "number"
      ? netbootXyz.tftp_host_port
      : Number(netbootXyz.tftp_host_port);
  if (Number.isFinite(p) && p >= 1 && p <= 65535) return Math.floor(p);
  return 69;
}
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listNetbootXyzDeploymentSummaries(cfg) {
  const { deployments } = normalizeNetbootXyzConfig(cfg);
  return deployments.map((d) => {
    ...
    const netbootXyz = isObject(d.netboot_xyz) ? d.netboot_xyz : {};
    const imageTag =
      typeof netbootXyz.image_tag === "string" && netbootXyz.image_tag.trim()
        ? netbootXyz.image_tag.trim()
        : "latest";
    return {
      ...
      image_tag: imageTag,
      menu_version:
        typeof netbootXyz.menu_version === "string" ? netbootXyz.menu_version : null,
      web_app_port: webAppPort(netbootXyz),
      nginx_host_port: nginxHostPort(netbootXyz),
      tftp_host_port: tftpHostPort(netbootXyz),
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(NETBOOT_XYZ_ROLE).test(t)) return t;
  return lxcSystemId(NETBOOT_XYZ_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    netboot_xyz: isObject(d.netboot_xyz) ? d.netboot_xyz : {},
    install,
  };
}
 ...
 */
export function resolveNetbootXyzDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeNetbootXyzConfig(cfg);
