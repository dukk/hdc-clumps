 ...
 *
 * Usage: hdc run service searxng maintain -- [--instance a | --system-id searxng-a]
...
import { resolveSearxngDeployments } from "hdc/package/deployments.mjs";
import { maintainSearxngInCt, resolvePveSshForHost } from "hdc/package/searxng-install.mjs";
 ...
 * @param {{ secret: string; skipUpgrade: boolean }} runOpts
 */
async function maintainOne(deployment, flags, vaultAccess, runOpts) {
  const { systemId, proxmox: px, searxng, install } = deployment;
  ...
  const result = await maintainSearxngInCt(
    pveSsh.user,
    pveSsh.host,
    vmid,
    searxngCfg,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: refresh SearXNG Docker stack (stderr log; JSON on stdout).\n`);
