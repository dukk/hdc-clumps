...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(SEARXNG_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.searxng)) defaults.searxng = structuredClone(cfg.searxng);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeSearxngConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("searxng config must be a JSON object");
  }
  ...
  throw new Error("searxng config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(SEARXNG_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match searxng-<letter>`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listSearxngDeploymentSummaries(cfg) {
  const { deployments } = normalizeSearxngConfig(cfg);
  return deployments.map((d) => {
    ...
    const searxng = isObject(d.searxng) ? d.searxng : {};
    return {
      ...
      image_tag: typeof searxng.image_tag === "string" ? searxng.image_tag : "latest",
      host_port: hostPort(searxng),
      instance_name: instanceName(searxng),
      limiter: limiterEnabled(searxng),
    };
  });
}
 ...
 */
export function hostPort(searxng) {
  const p = typeof searxng.host_port === "number" ? searxng.host_port : Number(searxng.host_port);
 ...
 * @param {Record<string, unknown>} searxng
 */
export function instanceName(searxng) {
  const n = typeof searxng.instance_name === "string" ? searxng.instance_name.trim() : "";
  return n || "SearXNG";
}
 ...
 * @param {Record<string, unknown>} searxng
 */
export function limiterEnabled(searxng) {
  return searxng.limiter === true;
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(SEARXNG_ROLE).test(t)) return t;
  return lxcSystemId(SEARXNG_ROLE, t);
}

/**
 * @param {Record<string, unknown>} searxng
 */
export function secretKeyVaultKey(searxng) {
  const key =
    typeof searxng.secret_key_vault_key === "string" && searxng.secret_key_vault_key.trim()
      ? searxng.secret_key_vault_key.trim()
      : "HDC_SEARXNG_SECRET";
  return key;
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    searxng: isObject(d.searxng) ? d.searxng : {},
    install,
  };
}
 ...
 */
export function resolveSearxngDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeSearxngConfig(cfg);
