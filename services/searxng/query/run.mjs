#!/usr/bin/env node
/**
 * Query SearXNG deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service searxng query -- [--instance a]
...
import { resolvePveSshForHost } from "hdc/package/searxng-install.mjs";
import { querySearxngInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeSearxngConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listSearxngDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveSearxngDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await querySearxngInCt(pveSsh.user, pveSsh.host, vmid, d.searxng, d.install);
