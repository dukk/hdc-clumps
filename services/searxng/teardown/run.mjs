#!/usr/bin/env node
/**
 * Teardown SearXNG Proxmox LXC deployments.
 *
 * Usage: hdc run service searxng teardown -- [--instance a | --system-id searxng-a]
 *        hdc run service searxng teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/searxng-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down SearXNG LXC (stderr log; JSON on stdout).\n`);
