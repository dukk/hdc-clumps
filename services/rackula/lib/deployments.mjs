...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(RACKULA_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.rackula)) defaults.rackula = structuredClone(cfg.rackula);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeRackulaConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("rackula config must be a JSON object");
  }
  ...
  throw new Error("rackula config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(RACKULA_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match rackula-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const rackula = isObject(d.rackula) ? d.rackula : {};
    const publicUrl = rackula.public_url;
    if (publicUrl !== null && publicUrl !== undefined) {
      const s = typeof publicUrl === "string" ? publicUrl.trim() : "";
      if (s) parsePublicUrl(rackula);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listRackulaDeploymentSummaries(cfg) {
  const { deployments } = normalizeRackulaConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      frontend_image: normalizeFrontendImage(rackula),
      api_image: normalizeApiImage(rackula),
      host_port: hostPort(rackula),
      public_url: publicUrl,
      api_write_token_enabled: apiWriteTokenEnabled(rackula),
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(RACKULA_ROLE).test(t)) return t;
  return lxcSystemId(RACKULA_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    rackula: isObject(d.rackula) ? d.rackula : {},
    install,
  };
}
 ...
 */
export function resolveRackulaDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeRackulaConfig(cfg);
