#!/usr/bin/env node
/**
 * Query Rackula deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service rackula query -- [--instance a]
...
import {
  listRackulaDeploymentSummaries,
  normalizeRackulaConfig,
  resolveRackulaDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/rackula-install.mjs";
import { queryRackulaInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeRackulaConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listRackulaDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveRackulaDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryRackulaInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.rackula,
