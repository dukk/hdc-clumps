#!/usr/bin/env node
/**
 * Deploy Rackula on Proxmox LXC (Docker Compose with persistence).
 *
 * Usage: hdc run service rackula deploy -- [--instance a | --system-id rackula-a] [--skip-install]
 *        hdc run service rackula deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { installRackulaInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/rackula-install.mjs";
import { apiWriteTokenEnabled, hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/rackula-render.mjs";
import { createRackulaVaultAccess, resolveApiWriteToken } from "hdc/package/vault-deps.mjs";
import {
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; vaultAccess?: import("hdc/cli/lib/vault-access.mjs").VaultAccess | null }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, rackula, install } = deployment;
  ...
  const rackulaCfg = isObject(rackula) ? rackula : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    let apiWriteToken = null;
    if (apiWriteTokenEnabled(rackulaCfg) && runOpts.vaultAccess) {
      apiWriteToken = await resolveApiWriteToken(runOpts.vaultAccess, rackulaCfg);
    }
    installResult = await installRackulaInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      rackulaCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(rackulaCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, rackulaCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(rackulaCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Rackula LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveRackulaDeployments>} */
  let deployments;
  try {
  ...
  const needsVault = deployments.some((d) => apiWriteTokenEnabled(d.rackula));
  /** @type {import("hdc/cli/lib/vault-access.mjs").VaultAccess | null} */
  let vaultAccess = null;
  if (needsVault) {
    vaultAccess = createRackulaVaultAccess();
  ...
  for (const deployment of deployments) {
    try {
      results.push(await deployOne(deployment, flags, log, { ctPasswordCache, vaultAccess }));
    } catch (e) {
      const msg = String(/** @type {Error} */ (e).message || e);
