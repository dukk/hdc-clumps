#!/usr/bin/env node
/**
 * Query draw.io deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service draw-io query -- [--instance a]
...
import {
  listDrawIoDeploymentSummaries,
  normalizeDrawIoConfig,
  resolveDrawIoDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/draw-io-install.mjs";
import { queryDrawIoInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeDrawIoConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listDrawIoDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveDrawIoDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryDrawIoInCt(pveSsh.user, pveSsh.host, vmid, d.drawIo, d.install);
