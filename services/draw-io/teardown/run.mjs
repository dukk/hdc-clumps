#!/usr/bin/env node
/**
 * Teardown draw.io Proxmox LXC deployments.
 *
 * Usage: hdc run service draw-io teardown -- [--instance a | --system-id draw-io-a]
 *        hdc run service draw-io teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/draw-io-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down draw.io LXC (stderr log; JSON on stdout).\n`);
