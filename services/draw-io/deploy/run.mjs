#!/usr/bin/env node
/**
 * Deploy draw.io on Proxmox LXC (Docker Compose, nginx-waf HTTPS public_url).
 *
 * Usage: hdc run service draw-io deploy -- [--instance a | --system-id draw-io-a] [--skip-install]
 *        hdc run service draw-io deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveDrawIoDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installDrawIoInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/draw-io-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/draw-io-render.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/draw-io/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveDrawIoDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, drawIo, install } = deployment;
  ...
  /** @type {{ ok: boolean; method?: string; message?: string; web_url?: string; upstream_url?: string | null } | null} */
  ...
  const drawIoCfg = isObject(drawIo) ? drawIo : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installDrawIoInCt(pveSsh.user, pveSsh.host, guestVmid, drawIoCfg, installCfg);
  } else {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    web_url: resolveWebUrl(drawIoCfg),
    upstream_url: resolveUpstreamUrl(ip, drawIoCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(drawIoCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: draw.io LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveDrawIoDeployments>} */
  let deployments;
  try {
    deployments = resolveDrawIoDeployments(cfg, flags);
  } catch (e) {
