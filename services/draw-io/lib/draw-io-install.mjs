 ...
 */
export async function installDrawIoInCt(user, pveHost, vmid, drawIo, install) {
  errout.write(`[hdc] draw-io install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "draw-io install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const envContent = renderDrawIoEnv(drawIo);
  ...
  errout.write(`[hdc] draw-io install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    web_url: resolveWebUrl(drawIo),
    upstream_url: resolveUpstreamUrl(ip, drawIo),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainDrawIoInCt(user, pveHost, vmid, drawIo, install, opts = {}) {
  errout.write(`[hdc] draw-io maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "draw-io maintain");
  if (!ready) {
