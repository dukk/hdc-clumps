...
import { hostPort, normalizeImageTag, normalizePublicUrl } from "./draw-io-render.mjs";

const DRAW_IO_ROLE = "draw-io";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(DRAW_IO_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.draw_io)) defaults.draw_io = structuredClone(cfg.draw_io);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeDrawIoConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("draw-io config must be a JSON object");
  }
  ...
  throw new Error("draw-io config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(DRAW_IO_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match draw-io-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const dio = isObject(d.draw_io) ? d.draw_io : {};
    normalizePublicUrl(dio);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listDrawIoDeploymentSummaries(cfg) {
  const { deployments } = normalizeDrawIoConfig(cfg);
  return deployments.map((d) => {
    ...
    let publicUrl = null;
    try {
      publicUrl = normalizePublicUrl(dio);
    } catch {
      publicUrl = null;
    }
    return {
      ...
      image_tag: normalizeImageTag(dio),
      host_port: hostPort(dio),
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(DRAW_IO_ROLE).test(t)) return t;
  return lxcSystemId(DRAW_IO_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    drawIo: isObject(d.draw_io) ? d.draw_io : {},
    install,
  };
}
 ...
 */
export function resolveDrawIoDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeDrawIoConfig(cfg);
