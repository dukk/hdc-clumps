#!/usr/bin/env node
/**
 * Teardown Audiobookshelf Proxmox QEMU deployments.
 *
 * Usage: hdc run service audiobookshelf teardown -- [--instance a | --system-id vm-audiobookshelf-a]
 *        hdc run service audiobookshelf teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownOnHost } from "hdc/package/audiobookshelf-install.mjs";
...
async function teardownOne(deployment, flags) {
  ...
  if (!skipComposeDown && isObject(configure)) {
    try {
      ...
      if (host) {
        ...
        errout.write(`[hdc] ${target} ${verb}: stopping Docker Compose on ${host} …\n`);
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Audiobookshelf QEMU (stderr log; JSON on stdout).\n`);
