#!/usr/bin/env node
/**
 * Query LiteLLM deployments (config summary + optional live CT status).
 *
...
import {
  listLitellmDeploymentSummaries,
  normalizeLitellmConfig,
  resolveLitellmDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/litellm-install.mjs";
import { queryLitellmInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeLitellmConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listLitellmDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveLitellmDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryLitellmInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.litellm,
