#!/usr/bin/env node
/**
 * Deploy LiteLLM on Proxmox LXC (Docker Compose + Postgres).
 *
 * Usage: hdc run service litellm deploy -- [--instance a | --system-id litellm-a] [--skip-install]
 *        hdc run service litellm deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { installLitellmInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/litellm-install.mjs";
import { hostPort, resolveApiUrl, resolveUiUrl, resolveUpstreamUrl } from "hdc/package/litellm-render.mjs";
import {
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; secrets: Awaited<ReturnType<typeof resolveLitellmSecrets>> }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, litellm, install } = deployment;
  ...
  /** @type {{ ok: boolean; method?: string; message?: string; api_url?: string | null; ui_url?: string | null; upstream_url?: string | null } | null} */
  ...
  if (shouldInstall(install)) {
    installResult = await installLitellmInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      litellmCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    api_url: resolveApiUrl(litellmCfg, ip),
    ui_url: resolveUiUrl(litellmCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, litellmCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(litellmCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: LiteLLM LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveLitellmDeployments>} */
  let deployments;
  try {
