#!/usr/bin/env node
/**
 * Teardown LiteLLM Proxmox LXC deployments.
 *
 * Usage: hdc run service litellm teardown -- [--instance a | --system-id litellm-a]
 *        hdc run service litellm teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/litellm-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down LiteLLM LXC (stderr log; JSON on stdout).\n`);
