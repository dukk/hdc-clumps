...
function buildDbAndModelsProbeScript(composeDirPath, port) {
  ...
  return [
    ...
    "",
    "def psql_ok_tcp(password):",
    "    '''TCP auth as Prisma uses (socket trust can mask POSTGRES_PASSWORD drift).'''",
    "    if not password:",
    "        return False",
    "    r = subprocess.run(",
    "        [",
    "            'docker', 'run', '--rm', '--network', 'litellm_default',",
    "            '-e', f'PGPASSWORD={password}',",
    "            'postgres:16',",
    "            'psql', '-h', 'db', '-U', 'llmproxy', '-d', 'litellm', '-c', 'select 1',",
    "        ],",
    "        capture_output=True,",
    "        text=True,",
    "    )",
    ...
    "        'psql_tcp_ok_with_file_password': None,",
    ...
    "",
    "out['db_auth']['psql_ok_with_file_password'] = psql_ok_socket(file_pw) if file_pw else False",
    "out['db_auth']['psql_ok_with_container_password'] = psql_ok_socket(db_pw) if db_pw else False",
    "out['db_auth']['psql_tcp_ok_with_file_password'] = psql_ok_tcp(file_pw) if file_pw else False",
    "",
