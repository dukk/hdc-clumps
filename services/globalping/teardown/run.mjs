#!/usr/bin/env node
/**
 * Teardown globalping Proxmox LXC deployments.
 *
 * Usage: hdc run service globalping teardown -- [--instance a | --system-id globalping-a]
 *        hdc run service globalping teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/globalping-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down globalping LXC (stderr log; JSON on stdout).\n`);
