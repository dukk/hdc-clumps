#!/usr/bin/env node
/**
 * Deploy globalping on Proxmox LXC (Docker Compose, host-network probe).
 *
 * Usage: hdc run service globalping deploy -- [--instance a | --system-id globalping-a] [--skip-install]
 *        hdc run service globalping deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveGlobalpingDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installGlobalpingInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/globalping-install.mjs";
import {
...
import { createGlobalpingVaultAccess, resolveAdoptionToken } from "hdc/package/vault-deps.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/globalping/config.example.json";
...
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, globalping, install } = deployment;
  ...
  /** @type {{ ok: boolean; method?: string; message?: string; ct_ip?: string | null } | null} */
  ...
  if (shouldInstall(install)) {
    if (!runOpts.vaultAccess) {
      return {
        ...
        host_id: hostId,
        mode,
        message: "vault access required for GP_ADOPTION_TOKEN",
      };
    }
    let adoptionToken;
    try {
      adoptionToken = await resolveAdoptionToken(runOpts.vaultAccess, globalpingCfg);
    } catch (e) {
      return {
        ok: false,
        system_id: systemId,
    ...
    installResult = await installGlobalpingInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      globalpingCfg,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: globalping LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveGlobalpingDeployments>} */
  let deployments;
  try {
  ...
  const needsVault = deployments.some((d) => shouldInstall(d.install));
  /** @type {import("hdc/cli/lib/vault-access.mjs").VaultAccess | null} */
  let vaultAccess = null;
  if (needsVault) {
    vaultAccess = createGlobalpingVaultAccess();
