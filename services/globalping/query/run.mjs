#!/usr/bin/env node
/**
 * Query globalping deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service globalping query -- [--instance a]
...
import {
  listGlobalpingDeploymentSummaries,
  normalizeGlobalpingConfig,
  resolveGlobalpingDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/globalping-install.mjs";
import { queryGlobalpingInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeGlobalpingConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listGlobalpingDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveGlobalpingDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryGlobalpingInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.globalping,
          ...
          liveResults.push({ system_id: d.systemId, ok: status.container_running, ...status });
        } catch (e) {
          liveResults.push({
            system_id: d.systemId,
            ok: false,
