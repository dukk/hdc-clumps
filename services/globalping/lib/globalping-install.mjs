 ...
 * @param {string} adoptionToken
 */
export async function installGlobalpingInCt(user, pveHost, vmid, globalping, install, adoptionToken) {
  errout.write(`[hdc] globalping install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "globalping install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const composeYaml = renderComposeYaml(globalping);
  const envContent = renderEnvFile(adoptionToken);
  ...
  errout.write(`[hdc] globalping install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
 ...
 * @param {Record<string, unknown>} globalping
 * @param {Record<string, unknown>} install
 * @param {{ skipUpgrade?: boolean; adoptionToken?: string }} [opts]
 */
export async function maintainGlobalpingInCt(user, pveHost, vmid, globalping, install, opts = {}) {
  errout.write(`[hdc] globalping maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "globalping maintain");
  if (!ready) {
    return { ok: false, message: `CT ${vmid} not reachable via pct exec` };
  }

  const token = opts.adoptionToken;
  if (!token || !String(token).trim()) {
    return { ok: false, message: "adoption token missing" };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const composeYaml = renderComposeYaml(globalping);
  const envContent = renderEnvFile(token);
  const dir = composeDir(install);
