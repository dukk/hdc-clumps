...
import { adoptionTokenVaultKey, normalizeImage } from "./globalping-render.mjs";

const GLOBALPING_ROLE = "globalping";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(GLOBALPING_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.globalping)) defaults.globalping = structuredClone(cfg.globalping);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeGlobalpingConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("globalping config must be a JSON object");
  }
  ...
  throw new Error("globalping config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(GLOBALPING_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match globalping-<letter>`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listGlobalpingDeploymentSummaries(cfg) {
  const { deployments } = normalizeGlobalpingConfig(cfg);
  return deployments.map((d) => {
    ...
    const globalping = isObject(d.globalping) ? d.globalping : {};
    return {
      ...
      image: normalizeImage(globalping),
      adoption_token_vault_key: adoptionTokenVaultKey(globalping),
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(GLOBALPING_ROLE).test(t)) return t;
  return lxcSystemId(GLOBALPING_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    globalping: isObject(d.globalping) ? d.globalping : {},
    install,
  };
}
 ...
 */
export function resolveGlobalpingDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeGlobalpingConfig(cfg);
