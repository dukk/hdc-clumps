#!/usr/bin/env node
/**
 * Teardown A2A Registry Proxmox LXC deployments.
 *
 * Usage: hdc run service a2a-registry teardown -- [--instance a | --system-id a2a-registry-a]
 *        hdc run service a2a-registry teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/a2a-registry-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down a2a-registry LXC (stderr log; JSON on stdout).\n`);
