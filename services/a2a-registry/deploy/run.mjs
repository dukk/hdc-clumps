#!/usr/bin/env node
/**
 * Deploy A2A Registry on Proxmox LXC (Docker Compose, local image build).
 *
 * Usage: hdc run service a2a-registry deploy -- [--instance a | --system-id a2a-registry-a] [--skip-install]
 *        hdc run service a2a-registry deploy -- [--skip-existing | --redeploy-existing]
 */
...
import {
  installA2aRegistryInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/a2a-registry-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/a2a-registry-render.mjs";
import {
...
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, a2a_registry, install } = deployment;
  ...
  if (shouldInstall(install)) {
    installResult = await installA2aRegistryInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      a2aRegistryCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(a2aRegistryCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, a2aRegistryCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(a2aRegistryCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: a2a-registry LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveA2aRegistryDeployments>} */
  let deployments;
  try {
