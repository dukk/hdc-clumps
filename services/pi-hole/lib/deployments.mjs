...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(PIHOLE_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.pihole)) defaults.pihole = structuredClone(cfg.pihole);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizePiHoleConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("pi-hole config must be a JSON object");
  }
  ...
  throw new Error("pi-hole config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(PIHOLE_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match pi-hole-<letter>`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listPiHoleDeploymentSummaries(cfg) {
  const { deployments } = normalizePiHoleConfig(cfg);
  return deployments.map((d) => {
    ...
    const pihole = isObject(d.pihole) ? d.pihole : {};
    return {
      ...
      upstream_dns: Array.isArray(pihole.upstream_dns) ? pihole.upstream_dns : [],
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(PIHOLE_ROLE).test(t)) return t;
  return lxcSystemId(PIHOLE_ROLE, t);
}

/**
 * Extract instance letter from pi-hole-a → a.
 * @param {string} systemId
 */
export function instanceLetterFromSystemId(systemId) {
  const m = new RegExp("^pi-hole-([a-z]+)$").exec(systemId.trim());
  return m ? m[1] : "";
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    pihole: isObject(d.pihole) ? d.pihole : {},
    install,
  };
}
 ...
 */
export function resolvePiHoleDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizePiHoleConfig(cfg);
