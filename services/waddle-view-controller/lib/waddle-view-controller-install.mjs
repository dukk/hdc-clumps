...
export function buildMaintainScript(composeDirPath, composeYaml, envContent, opts = {}) {
  ...
  if (!opts.skipUpgrade) {
    // Local-only tags (release tarball) have no registry; pull failure must not block up.
    lines.push("docker compose pull || true");
  }
 ...
 */
export async function installWaddleViewControllerInCt(
  user,
  pveHost,
  vmid,
  waddleCfg,
  ...
  errout.write(`[hdc] waddle-view-controller install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "waddle-view-controller install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const envContent = renderWaddleViewControllerEnv(waddleCfg, sessionSecret);
  const composeYaml = renderComposeYaml(waddleCfg);
  ...
  errout.write(`[hdc] waddle-view-controller install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(waddleCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, waddleCfg),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainWaddleViewControllerInCt(
  user,
  pveHost,
  vmid,
  waddleCfg,
  ...
  errout.write(`[hdc] waddle-view-controller maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "waddle-view-controller maintain");
  if (!ready) {
