...
import {
  ...
  parsePublicUrl,
  resolveImageRef,
  sessionSecretVaultKey,
} from "./waddle-view-controller-render.mjs";

const WADDLE_VIEW_CONTROLLER_ROLE = "waddle-view-controller";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(WADDLE_VIEW_CONTROLLER_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.waddle_view_controller)) {
    defaults.waddle_view_controller = structuredClone(cfg.waddle_view_controller);
  }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeWaddleViewControllerConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("waddle-view-controller config must be a JSON object");
  }
  ...
  throw new Error("waddle-view-controller config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(WADDLE_VIEW_CONTROLLER_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match waddle-view-controller-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const waddle = isObject(d.waddle_view_controller) ? d.waddle_view_controller : {};
    const publicUrl = waddle.public_url;
    if (publicUrl !== null && publicUrl !== undefined) {
      const s = typeof publicUrl === "string" ? publicUrl.trim() : "";
      if (s) parsePublicUrl(waddle);
    }
    if (authEnabled(waddle) && !sessionSecretVaultKey(waddle)) {
      throw new Error(
        `${sid}: waddle_view_controller.session_secret_vault_key required when auth_enabled`,
      );
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listWaddleViewControllerDeploymentSummaries(cfg) {
  const { deployments } = normalizeWaddleViewControllerConfig(cfg);
  return deployments.map((d) => {
    ...
    let publicUrl = null;
    try {
      const parsed = parsePublicUrl(waddle);
    ...
    return {
      ...
      image: resolveImageRef(waddle),
      host_port: hostPort(waddle),
      public_url: publicUrl,
      auth_enabled: authEnabled(waddle),
      session_secret_vault_key: sessionSecretVaultKey(waddle),
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(WADDLE_VIEW_CONTROLLER_ROLE).test(t)) return t;
  return lxcSystemId(WADDLE_VIEW_CONTROLLER_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    waddle_view_controller: isObject(d.waddle_view_controller) ? d.waddle_view_controller : {},
    install,
  };
}
 ...
 */
export function resolveWaddleViewControllerDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeWaddleViewControllerConfig(cfg);
