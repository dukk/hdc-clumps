#!/usr/bin/env node
/**
 * Query waddle-view-controller deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service waddle-view-controller query -- [--instance a]
...
import {
  listWaddleViewControllerDeploymentSummaries,
  normalizeWaddleViewControllerConfig,
  resolveWaddleViewControllerDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/waddle-view-controller-install.mjs";
import { queryWaddleViewControllerInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeWaddleViewControllerConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listWaddleViewControllerDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveWaddleViewControllerDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryWaddleViewControllerInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.waddle_view_controller,
            d.install,
          );
