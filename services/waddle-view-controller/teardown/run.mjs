#!/usr/bin/env node
/**
 * Teardown waddle-view-controller Proxmox LXC deployments.
 *
 * Usage: hdc run service waddle-view-controller teardown -- [--instance a | --system-id waddle-view-controller-a]
 *        hdc run service waddle-view-controller teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/waddle-view-controller-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down waddle-view-controller LXC (stderr log; JSON on stdout).\n`);
