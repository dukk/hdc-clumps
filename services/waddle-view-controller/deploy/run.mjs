#!/usr/bin/env node
/**
 * Deploy waddle-view-controller on Proxmox LXC (Docker Compose).
 *
 * Usage: hdc run service waddle-view-controller deploy -- [--instance a | --system-id waddle-view-controller-a] [--skip-install]
 *        hdc run service waddle-view-controller deploy -- [--skip-existing | --redeploy-existing]
 */
...
import {
  installWaddleViewControllerInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/waddle-view-controller-install.mjs";
import {
  ...
  resolveUpstreamUrl,
  resolveWebUrl,
} from "hdc/package/waddle-view-controller-render.mjs";
import {
...
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, waddle_view_controller, install } = deployment;
  ...
  if (shouldInstall(install)) {
    ...
    if (authEnabled(waddleCfg)) {
      ...
      sessionSecret = String(
        await runOpts.vault.getSecret(sessionKey, { promptLabel: `vault secret ${sessionKey}` }),
      ).trim();
      if (!sessionSecret) {
        return {
          ok: false,
          ...
          host_id: hostId,
          mode,
          message: `missing vault ${sessionKey}`,
        };
      }
    }
    installResult = await installWaddleViewControllerInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      waddleCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(waddleCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, waddleCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(waddleCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: waddle-view-controller LXC via Proxmox (stderr log; JSON on stdout).\n`);
