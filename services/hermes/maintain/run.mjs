#!/usr/bin/env node
/**
 * Maintain Hermes Agent: refresh Docker images and recreate containers.
 *
...
import { resolveHermesDeployments } from "hdc/package/deployments.mjs";
import {
  maintainHermesInCt,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: refresh Hermes Docker stack (stderr log; JSON on stdout).\n`);
