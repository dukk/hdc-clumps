#!/usr/bin/env node
/**
 * Teardown GitLab CE Proxmox LXC deployments.
 *
 * Usage: hdc run service gitlab teardown -- [--instance a | --system-id gitlab-a]
 *        hdc run service gitlab teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/gitlab-install.mjs";
import {
  confirmTeardown,
  teardownDryRun,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down GitLab LXC (stderr log; JSON on stdout).\n`);
