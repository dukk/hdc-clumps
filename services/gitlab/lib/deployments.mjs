...
import { hostPort, normalizeExternalUrl, normalizeImageTag, sshHostPort } from "./gitlab-render.mjs";

const GITLAB_ROLE = "gitlab";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(GITLAB_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.gitlab)) defaults.gitlab = structuredClone(cfg.gitlab);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeGitlabConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("gitlab config must be a JSON object");
  }
  ...
  throw new Error("gitlab config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(GITLAB_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match gitlab-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const gl = isObject(d.gitlab) ? d.gitlab : {};
    normalizeExternalUrl(gl);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listGitlabDeploymentSummaries(cfg) {
  const { deployments } = normalizeGitlabConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(gl),
      host_port: hostPort(gl),
      ssh_host_port: sshHostPort(gl),
      external_url: externalUrl,
      signups_enabled: gl.signups_enabled === true,
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(GITLAB_ROLE).test(t)) return t;
  return lxcSystemId(GITLAB_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    gitlab: isObject(d.gitlab) ? d.gitlab : {},
    install,
  };
}
 ...
 */
export function resolveGitlabDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeGitlabConfig(cfg);
