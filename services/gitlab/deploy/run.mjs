#!/usr/bin/env node
/**
 * Deploy GitLab CE on Proxmox LXC (Docker Compose, nginx-waf HTTPS external_url).
 *
 * Usage: hdc run service gitlab deploy -- [--instance a | --system-id gitlab-a] [--skip-install]
 *        hdc run service gitlab deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveGitlabDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import {
  installGitlabInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/gitlab-install.mjs";
import {
  hostPort,
  resolveSshCloneHint,
  resolveUpstreamUrl,
  resolveWebUrl,
} from "hdc/package/gitlab-render.mjs";
...
/**
 * @param {ReturnType<typeof resolveGitlabDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, gitlab, install } = deployment;
  ...
  const gitlabCfg = isObject(gitlab) ? gitlab : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installGitlabInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      gitlabCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    web_url: resolveWebUrl(gitlabCfg),
    upstream_url: resolveUpstreamUrl(ip, gitlabCfg) ?? installResult.upstream_url ?? null,
    ssh_clone_hint: resolveSshCloneHint(ip, gitlabCfg) ?? installResult.ssh_clone_hint ?? null,
    host_port: hostPort(gitlabCfg),
    initial_root_password_available: installResult.initial_root_password_available ?? null,
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: GitLab CE LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveGitlabDeployments>} */
  let deployments;
  try {
    deployments = resolveGitlabDeployments(cfg, flags);
  } catch (e) {
