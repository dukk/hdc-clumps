#!/usr/bin/env node
/**
 * Maintain GitLab CE: re-push Omnibus config, refresh Docker images, guest baseline.
 *
 * Usage: hdc run service gitlab maintain -- [--instance a | --system-id gitlab-a]
 *        hdc run service gitlab maintain -- [--skip-upgrade] [--skip-clamav]
 */
...
import { maintainGitlabInCt, resolvePveSshForHost } from "hdc/package/gitlab-install.mjs";
...
async function maintainOne(deployment, flags, vaultAccess) {
  const { systemId, proxmox: px, gitlab, install } = deployment;
  ...
  const result = await maintainGitlabInCt(
    pveSsh.user,
    pveSsh.host,
    vmid,
    gitlabCfg,
  ...
  return {
    ...
    ssh_clone_hint: result.ssh_clone_hint ?? null,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: refresh GitLab Docker stack (stderr log; JSON on stdout).\n`);
