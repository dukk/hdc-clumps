#!/usr/bin/env node
/**
 * Query docuseal deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service docuseal query -- [--instance a]
...
import {
  listDocusealDeploymentSummaries,
  normalizeDocusealConfig,
  resolveDocusealDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/docuseal-install.mjs";
import { queryDocusealInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeDocusealConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listDocusealDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveDocusealDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryDocusealInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.docuseal,
