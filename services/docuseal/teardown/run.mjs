#!/usr/bin/env node
/**
 * Teardown docuseal Proxmox LXC deployments.
 *
 * Usage: hdc run service docuseal teardown -- [--instance a | --system-id docuseal-a]
 *        hdc run service docuseal teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/docuseal-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down docuseal LXC (stderr log; JSON on stdout).\n`);
