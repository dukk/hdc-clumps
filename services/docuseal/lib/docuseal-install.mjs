 ...
 */
export async function installDocusealInCt(user, pveHost, vmid, docuseal, install, secrets) {
  errout.write(`[hdc] docuseal install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "docuseal install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const envContent = renderDocusealEnv(docuseal, secrets, ip);
  ...
  errout.write(`[hdc] docuseal install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(docuseal, ip),
    upstream_url: resolveUpstreamUrl(ip, docuseal),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainDocusealInCt(user, pveHost, vmid, docuseal, install, secrets, opts = {}) {
  errout.write(`[hdc] docuseal maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "docuseal maintain");
  if (!ready) {
