...
import {
  ...
  parsePublicUrl,
  secretKeyBaseVaultKey,
} from "./docuseal-render.mjs";

const DOCUSEAL_ROLE = "docuseal";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(DOCUSEAL_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.docuseal)) defaults.docuseal = structuredClone(cfg.docuseal);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeDocusealConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("docuseal config must be a JSON object");
  }
  ...
  throw new Error("docuseal config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(DOCUSEAL_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match docuseal-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const docuseal = isObject(d.docuseal) ? d.docuseal : {};
    const publicUrl = typeof docuseal.public_url === "string" ? docuseal.public_url.trim() : "";
    if (publicUrl) {
      parsePublicUrl(docuseal);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listDocusealDeploymentSummaries(cfg) {
  const { deployments } = normalizeDocusealConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(docuseal),
      postgres_image_tag: normalizePostgresImageTag(docuseal),
      host_port: hostPort(docuseal),
      public_url: publicUrl,
      timezone: typeof docuseal.timezone === "string" ? docuseal.timezone : null,
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(DOCUSEAL_ROLE).test(t)) return t;
  return lxcSystemId(DOCUSEAL_ROLE, t);
}

/**
 * @param {Record<string, unknown>} docuseal
 */
export function secretKeyBaseVaultKeyFromConfig(docuseal) {
  return secretKeyBaseVaultKey(isObject(docuseal) ? docuseal : {});
}

/**
 * @param {Record<string, unknown>} docuseal
 */
export function dbPasswordVaultKeyFromConfig(docuseal) {
  return dbPasswordVaultKey(isObject(docuseal) ? docuseal : {});
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    docuseal: isObject(d.docuseal) ? d.docuseal : {},
    install,
  };
}
 ...
 */
export function resolveDocusealDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeDocusealConfig(cfg);
