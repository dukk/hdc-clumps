...
import { composeDir, hostPort, parsePublicUrl } from "./docuseal-render.mjs";
 ...
 */
export async function queryDocusealInCt(user, pveHost, vmid, docuseal, install) {
  ...
  if (docker.stdout.trim() === "active") {
    const probeUrl = publicUrl || `http://127.0.0.1:${port}/`;
    const h = pctExec(
      user,
      pveHost,
      vmid,
