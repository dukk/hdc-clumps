#!/usr/bin/env node
/**
 * Deploy docuseal on Proxmox LXC (Docker Compose + PostgreSQL).
 *
 * Usage: hdc run service docuseal deploy -- [--instance a | --system-id docuseal-a] [--skip-install]
 *        hdc run service docuseal deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveDocusealDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installDocusealInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/docuseal-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/docuseal-render.mjs";
import {
...
import { createDocusealVaultAccess } from "hdc/package/docuseal-vault-deps.mjs";
import { resolveDocusealSecrets } from "hdc/package/vault-secrets.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/docuseal/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveDocusealDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null }; secrets: { secretKeyBase: string; dbPassword: string } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, docuseal, install } = deployment;
  ...
  const docusealCfg = isObject(docuseal) ? docuseal : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installDocusealInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      docusealCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(docusealCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, docusealCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(docusealCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: docuseal LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveDocusealDeployments>} */
  let deployments;
  try {
    deployments = resolveDocusealDeployments(cfg, flags);
  } catch (e) {
  ...
  const vault = createDocusealVaultAccess();
  const defaultsDocuseal =
    isObject(cfg.defaults) && isObject(cfg.defaults.docuseal) ? cfg.defaults.docuseal : {};
  let secrets;
  try {
    secrets = await resolveDocusealSecrets(vault, defaultsDocuseal);
  } catch (e) {
    const msg = String(/** @type {Error} */ (e).message || e);
