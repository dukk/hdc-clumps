#!/usr/bin/env node
/**
 * Query shlink deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service shlink query -- [--instance a]
...
import {
  listShlinkDeploymentSummaries,
  normalizeShlinkConfig,
  resolveShlinkDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/shlink-install.mjs";
import { queryShlinkInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeShlinkConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listShlinkDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveShlinkDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryShlinkInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.shlink,
