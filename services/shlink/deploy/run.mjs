#!/usr/bin/env node
/**
 * Deploy shlink on Proxmox LXC (Docker Compose + PostgreSQL + Redis + optional web client).
 *
 * Usage: hdc run service shlink deploy -- [--instance a | --system-id shlink-a] [--skip-install]
 *        hdc run service shlink deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveShlinkDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installShlinkInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/shlink-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/shlink-render.mjs";
import {
...
import { createShlinkVaultAccess } from "hdc/package/shlink-vault-deps.mjs";
import { resolveShlinkSecrets } from "hdc/package/vault-secrets.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/shlink/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveShlinkDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null }; secrets: { dbPassword: string; initialApiKey: string; geoliteLicenseKey?: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, shlink, install } = deployment;
  ...
  const shlinkCfg = isObject(shlink) ? shlink : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installShlinkInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      shlinkCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(shlinkCfg, ip),
    web_client_url: installResult.web_client_url ?? null,
    upstream_url: resolveUpstreamUrl(ip, shlinkCfg) ?? installResult.upstream_url ?? null,
    web_client_upstream_url: installResult.web_client_upstream_url ?? null,
    host_port: hostPort(shlinkCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: shlink LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveShlinkDeployments>} */
  let deployments;
  try {
    deployments = resolveShlinkDeployments(cfg, flags);
  } catch (e) {
  ...
  const vault = createShlinkVaultAccess();
  const defaultsShlink =
    isObject(cfg.defaults) && isObject(cfg.defaults.shlink) ? cfg.defaults.shlink : {};
  let secrets;
  try {
    secrets = await resolveShlinkSecrets(vault, defaultsShlink);
  } catch (e) {
    const msg = String(/** @type {Error} */ (e).message || e);
