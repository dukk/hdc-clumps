#!/usr/bin/env node
/**
 * Teardown shlink Proxmox LXC deployments.
 *
 * Usage: hdc run service shlink teardown -- [--instance a | --system-id shlink-a]
 *        hdc run service shlink teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/shlink-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down shlink LXC (stderr log; JSON on stdout).\n`);
