...
import { maintainShlinkInCt, resolvePveSshForHost } from "hdc/package/shlink-install.mjs";
...
async function maintainOne(deployment, flags, secrets, vaultAccess) {
  const { systemId, proxmox: px, shlink, install } = deployment;
  ...
  const result = await maintainShlinkInCt(
    pveSsh.user,
    pveSsh.host,
    vmid,
    shlinkCfg,
  ...
  return {
    ...
    url: result.url ?? null,
    web_client_url: result.web_client_url ?? null,
    upstream_url: result.upstream_url ?? null,
    web_client_upstream_url: result.web_client_upstream_url ?? null,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: refresh shlink Docker stack (stderr log; JSON on stdout).\n`);
