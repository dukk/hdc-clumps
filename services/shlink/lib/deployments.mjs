...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(SHLINK_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.shlink)) defaults.shlink = structuredClone(cfg.shlink);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeShlinkConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("shlink config must be a JSON object");
  }
  ...
  throw new Error("shlink config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(SHLINK_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match shlink-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const shlink = isObject(d.shlink) ? d.shlink : {};
    normalizeDefaultDomain(shlink);
    const publicUrl = typeof shlink.public_url === "string" ? shlink.public_url.trim() : "";
    if (publicUrl) {
      parsePublicUrl(shlink);
      validateDomainConsistency(shlink);
    }
    webClientConfig(shlink);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listShlinkDeploymentSummaries(cfg) {
  const { deployments } = normalizeShlinkConfig(cfg);
  return deployments.map((d) => {
    ...
    try {
      ...
      defaultDomain = normalizeDefaultDomain(shlink);
    } catch {
      publicUrl = null;
      defaultDomain = null;
    }
    ...
    return {
      ...
      image_tag: normalizeImageTag(shlink),
      postgres_image_tag: normalizePostgresImageTag(shlink),
      redis_image_tag: normalizeRedisImageTag(shlink),
      host_port: hostPort(shlink),
      default_domain: defaultDomain,
      public_url: publicUrl,
      web_client_enabled: wc.enabled,
      web_client_host_port: wc.enabled ? wc.host_port : null,
      web_client_public_url: wc.public_url ? wc.public_url.origin.replace(/\/+$/, "") : null,
      timezone: typeof shlink.timezone === "string" ? shlink.timezone : null,
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(SHLINK_ROLE).test(t)) return t;
  return lxcSystemId(SHLINK_ROLE, t);
}

/**
 * @param {Record<string, unknown>} shlink
 */
export function dbPasswordVaultKeyFromConfig(shlink) {
  return dbPasswordVaultKey(isObject(shlink) ? shlink : {});
}

/**
 * @param {Record<string, unknown>} shlink
 */
export function initialApiKeyVaultKeyFromConfig(shlink) {
  return initialApiKeyVaultKey(isObject(shlink) ? shlink : {});
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    shlink: isObject(d.shlink) ? d.shlink : {},
    install,
  };
}
 ...
 */
export function resolveShlinkDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeShlinkConfig(cfg);
