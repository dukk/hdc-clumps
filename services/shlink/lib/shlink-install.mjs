 ...
 */
export async function installShlinkInCt(user, pveHost, vmid, shlink, install, secrets) {
  errout.write(`[hdc] shlink install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "shlink install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const envContent = renderShlinkEnv(shlink, secrets);
  const composeYaml = renderComposeYaml(shlink);
  ...
  errout.write(`[hdc] shlink install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(shlink, ip),
    web_client_url: resolveWebClientUrl(shlink, ip),
    upstream_url: resolveUpstreamUrl(ip, shlink),
    web_client_upstream_url: resolveWebClientUpstreamUrl(ip, shlink),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainShlinkInCt(user, pveHost, vmid, shlink, install, secrets, opts = {}) {
  errout.write(`[hdc] shlink maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "shlink maintain");
  if (!ready) {
