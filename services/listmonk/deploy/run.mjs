#!/usr/bin/env node
/**
 * Deploy listmonk on Proxmox LXC (Docker Compose + PostgreSQL).
 *
 * Usage: hdc run service listmonk deploy -- [--instance a | --system-id listmonk-a] [--skip-install]
 *        hdc run service listmonk deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveListmonkDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installListmonkInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/listmonk-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/listmonk-render.mjs";
import {
...
import { createListmonkVaultAccess } from "hdc/package/listmonk-vault-deps.mjs";
import { resolveListmonkSecrets } from "hdc/package/vault-secrets.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/listmonk/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveListmonkDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null }; secrets: { adminPassword: string; dbPassword: string } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, listmonk, install } = deployment;
  ...
  const listmonkCfg = isObject(listmonk) ? listmonk : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installListmonkInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      listmonkCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(listmonkCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, listmonkCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(listmonkCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: listmonk LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveListmonkDeployments>} */
  let deployments;
  try {
    deployments = resolveListmonkDeployments(cfg, flags);
  } catch (e) {
  ...
  const vault = createListmonkVaultAccess();
  const defaultsListmonk =
    isObject(cfg.defaults) && isObject(cfg.defaults.listmonk) ? cfg.defaults.listmonk : {};
  let secrets;
  try {
    secrets = await resolveListmonkSecrets(vault, defaultsListmonk);
  } catch (e) {
    const msg = String(/** @type {Error} */ (e).message || e);
