#!/usr/bin/env node
/**
 * Query listmonk deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service listmonk query -- [--instance a]
...
import {
  listListmonkDeploymentSummaries,
  normalizeListmonkConfig,
  resolveListmonkDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/listmonk-install.mjs";
import { queryListmonkInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeListmonkConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listListmonkDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveListmonkDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryListmonkInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.listmonk,
