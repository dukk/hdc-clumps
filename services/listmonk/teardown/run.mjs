#!/usr/bin/env node
/**
 * Teardown listmonk Proxmox LXC deployments.
 *
 * Usage: hdc run service listmonk teardown -- [--instance a | --system-id listmonk-a]
 *        hdc run service listmonk teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/listmonk-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down listmonk LXC (stderr log; JSON on stdout).\n`);
