 ...
 */
export async function installListmonkInCt(user, pveHost, vmid, listmonk, install, secrets) {
  errout.write(`[hdc] listmonk install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "listmonk install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const envContent = renderListmonkEnv(listmonk, secrets, ip);
  ...
  errout.write(`[hdc] listmonk install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(listmonk, ip),
    upstream_url: resolveUpstreamUrl(ip, listmonk),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainListmonkInCt(user, pveHost, vmid, listmonk, install, secrets, opts = {}) {
  errout.write(`[hdc] listmonk maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "listmonk maintain");
  if (!ready) {
