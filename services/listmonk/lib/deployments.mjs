...
import {
  ...
  parsePublicUrl,
} from "./listmonk-render.mjs";

const LISTMONK_ROLE = "listmonk";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(LISTMONK_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.listmonk)) defaults.listmonk = structuredClone(cfg.listmonk);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeListmonkConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("listmonk config must be a JSON object");
  }
  ...
  throw new Error("listmonk config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(LISTMONK_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match listmonk-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const listmonk = isObject(d.listmonk) ? d.listmonk : {};
    const publicUrl = typeof listmonk.public_url === "string" ? listmonk.public_url.trim() : "";
    if (publicUrl) {
      parsePublicUrl(listmonk);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listListmonkDeploymentSummaries(cfg) {
  const { deployments } = normalizeListmonkConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(listmonk),
      postgres_image_tag: normalizePostgresImageTag(listmonk),
      host_port: hostPort(listmonk),
      public_url: publicUrl,
      timezone: typeof listmonk.timezone === "string" ? listmonk.timezone : null,
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(LISTMONK_ROLE).test(t)) return t;
  return lxcSystemId(LISTMONK_ROLE, t);
}

/**
 * @param {Record<string, unknown>} listmonk
 */
export function adminPasswordVaultKeyFromConfig(listmonk) {
  return adminPasswordVaultKey(isObject(listmonk) ? listmonk : {});
}

/**
 * @param {Record<string, unknown>} listmonk
 */
export function dbPasswordVaultKeyFromConfig(listmonk) {
  return dbPasswordVaultKey(isObject(listmonk) ? listmonk : {});
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    listmonk: isObject(d.listmonk) ? d.listmonk : {},
    install,
  };
}
 ...
 */
export function resolveListmonkDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeListmonkConfig(cfg);
