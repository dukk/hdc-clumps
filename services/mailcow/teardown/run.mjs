#!/usr/bin/env node
/**
 * Teardown Mailcow Proxmox LXC or QEMU deployments.
 *
 * Usage: hdc run service mailcow teardown -- [--instance a | --system-id vm-mailcow-a]
 *        hdc run service mailcow teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
async function teardownOne(deployment, flags) {
  ...
  const isQemu = mode === "proxmox-qemu" || mode === "configure-only";
  const vmid = isQemu
    ? (() => {
  ...
  if (!skipComposeDown) {
    try {
      if (isQemu) {
        ...
        if (host) {
          errout.write(`[hdc] ${target} ${verb}: stopping mailcow compose on ${host} …\n`);
          const exec = createConfigureExec("ssh", { user, host });
          composeDownOnHost(exec, installCfg);
        }
      } else {
        const pveSsh = resolvePveSshForHost(proxmoxRoot, hostId);
        errout.write(`[hdc] ${target} ${verb}: stopping mailcow compose in CT ${vmid} …\n`);
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Mailcow guests (stderr log; JSON on stdout).\n`);
