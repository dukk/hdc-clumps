 ...
 * @param {number} [vmid]
 */
export async function waitForLlamaCppProvisionTask(provisionResult, auth, systemId, vmid) {
  const upid = extractPveUpid(provisionResult.details?.task);
  if (!upid) {
    errout.write(
      `[hdc] llama-cpp deploy: ${systemId}: no Proxmox task UPID in provision result — continuing without task wait.\n`,
    );
    return;
  }
  const vmidLabel = vmid !== undefined ? ` (vmid ${vmid})` : "";
  errout.write(`[hdc] llama-cpp deploy: ${systemId}${vmidLabel}: waiting for Proxmox task to finish …\n`);
  await waitForPveTask({
    ...
    timeoutMs: 0,
    log: (line) => {
      errout.write(`[hdc] llama-cpp deploy: ${systemId}: ${line}\n`);
    },
  });
  errout.write(`[hdc] llama-cpp deploy: ${systemId}${vmidLabel}: Proxmox task completed.\n`);
}
