#!/usr/bin/env node
/**
 * Deploy Nagios 4 on Proxmox LXC; checks generated from BIND forward A records.
 *
 * Usage: hdc run service nagios deploy -- [--instance a | --system-id nagios-a] [--skip-install]
 *        hdc run service nagios deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { installNagiosInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/nagios-install.mjs";
import { configureNagiosInCt } from "hdc/package/nagios-configure.mjs";
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; nagiosCfg: string; bindPath: string }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, install } = deployment;
  const { nagiosCfg, bindPath } = runOpts;
  ...
  errout.write(
    `[hdc] ${target} ${verb}: ${JSON.stringify(systemId)} on ${JSON.stringify(hostId)} (BIND ${bindPath}) …\n`,
  );
  ...
  if (shouldInstall(install)) {
    installResult = await installNagiosInCt(pveSsh.user, pveSsh.host, vmid);
  } else {
  ...
  configureResult = configureNagiosInCt(pveSsh.user, pveSsh.host, vmid, nagiosCfg);
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Nagios LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  try {
    ...
    errout.write(
      `[hdc] ${target} ${verb}: BIND ${bindBundle.bindPath} → ${bindBundle.stats.hostCount} hosts, ${bindBundle.stats.serviceCount} services.\n`,
    );
  } catch (e) {
    errout.write(`[hdc] ${target} ${verb}: ${/** @type {Error} */ (e).message}\n`);
    process.stdout.write(
    ...
    try {
      results.push(
        await deployOne(deployment, flags, log, {
          ctPasswordCache,
          nagiosCfg: bindBundle.nagiosCfg,
          bindPath: bindBundle.bindPath,
        }),
      );
    } catch (e) {
      const msg = String(/** @type {Error} */ (e).message || e);
