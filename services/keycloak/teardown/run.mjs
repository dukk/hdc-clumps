#!/usr/bin/env node
/**
 * Teardown Keycloak Proxmox LXC deployments.
 *
 * Usage: hdc run service keycloak teardown -- [--instance a | --system-id keycloak-a]
 *        hdc run service keycloak teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { resolveKeycloakDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/keycloak-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Keycloak LXC (stderr log; JSON on stdout).\n`);
