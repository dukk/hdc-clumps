...
function normalizeV1(cfg) {
  ...
  if (isObject(cfg.keycloak)) defaults.keycloak = structuredClone(cfg.keycloak);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeKeycloakConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("keycloak config must be a JSON object");
  }
  ...
  throw new Error("keycloak config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match keycloak-<letter>`);
    }
    ...
    const keycloak = isObject(d.keycloak) ? d.keycloak : {};
    normalizeExternalUrl(keycloak);
    hostPort(keycloak);
    imageTag(keycloak);
    adminPasswordVaultKey(keycloak);
    const db = databaseConfig(keycloak);
    if (db.mode === "external" && !db.external.host) {
      throw new Error(`${sid}: keycloak.database.external.host required when mode=external`);
    }
  }
}
 ...
 */
export function databasePasswordVaultKey(keycloak) {
  const db = databaseConfig(keycloak);
  return db.mode === "bundled" ? db.bundled.postgres_password_vault_key : db.external.password_vault_key;
}

/**
 * @param {Record<string, unknown>} cfg
 */
export function listKeycloakDeploymentSummaries(cfg) {
  const { deployments } = normalizeKeycloakConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      host_port: hostPort(keycloak),
      image_tag: imageTag(keycloak),
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    keycloak: isObject(d.keycloak) ? d.keycloak : {},
    install,
  };
}
 ...
 */
export function resolveKeycloakDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeKeycloakConfig(cfg);
