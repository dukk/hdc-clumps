...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(HOMEPAGE_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.homepage)) defaults.homepage = structuredClone(cfg.homepage);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeHomepageConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("homepage config must be a JSON object");
  }
  ...
  throw new Error("homepage config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(HOMEPAGE_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match homepage-<letter>`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listHomepageDeploymentSummaries(cfg) {
  const { deployments } = normalizeHomepageConfig(cfg);
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(HOMEPAGE_ROLE).test(t)) return t;
  return lxcSystemId(HOMEPAGE_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    homepage: isObject(d.homepage) ? d.homepage : {},
    install,
  };
}
 ...
 */
export function resolveHomepageDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeHomepageConfig(cfg);
