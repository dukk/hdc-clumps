#!/usr/bin/env node
/**
 * Teardown Homepage Proxmox LXC deployments.
 *
 * Usage: hdc run service homepage teardown -- [--instance a | --system-id homepage-a]
 *        hdc run service homepage teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/homepage-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Homepage LXC (stderr log; JSON on stdout).\n`);
