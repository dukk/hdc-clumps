#!/usr/bin/env node
/**
 * Deploy Homepage on Proxmox LXC (Docker Compose, nginx-waf HTTPS public_url).
 *
 * Usage: hdc run service homepage deploy -- [--instance a | --system-id homepage-a] [--skip-install]
 *        hdc run service homepage deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { installHomepageInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/homepage-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/homepage-render.mjs";
...
async function deployOne(deployment, flags, log, runOpts) {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    web_url: resolveWebUrl(homepageCfg),
    upstream_url: resolveUpstreamUrl(ip, homepageCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(homepageCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Homepage LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveHomepageDeployments>} */
  let deployments;
  try {
