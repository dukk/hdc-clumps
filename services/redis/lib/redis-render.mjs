/**
 * Render redis.conf for cluster mode. Password lines are optional so tests can assert structure without secrets.
 *
 * @param {object} opts
 ...
 */
export function renderRedisConf(opts) {
  ...
  if (!announceIp) {
    throw new Error("renderRedisConf: announceIp required");
  }

  const lines = [
    "# Managed by hdc redis package",
    "bind 0.0.0.0",
    "protected-mode yes",
    `port ${port}`,
    "tcp-backlog 511",
