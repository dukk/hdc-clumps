...
import { aptInstallRedisCommand } from "./redis-install.mjs";
import { renderRedisConf } from "./redis-render.mjs";
...
export function configureRedis(opts) {
  ...
  uploadFile(exec, "/etc/redis/redis.conf", conf, log);
  runChecked(exec, "chown redis:redis /etc/redis/redis.conf", log);
  runChecked(exec, "systemctl enable redis-server", log);
  runChecked(exec, "systemctl restart redis-server", log);
  runChecked(
    exec,
    `redis-cli -a ${shellQuote(password)} -p ${port} ping 2>/dev/null | grep -q PONG`,
    log,
  );

  return {
    ok: true,
    message: `Redis configured (${exec.label})`,
