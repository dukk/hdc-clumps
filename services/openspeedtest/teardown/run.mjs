#!/usr/bin/env node
/**
 * Teardown openspeedtest Proxmox LXC deployments.
 *
 * Usage: hdc run service openspeedtest teardown -- [--instance a | --system-id openspeedtest-a]
 *        hdc run service openspeedtest teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/openspeedtest-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down openspeedtest LXC (stderr log; JSON on stdout).\n`);
