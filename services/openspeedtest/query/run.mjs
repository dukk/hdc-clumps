#!/usr/bin/env node
/**
 * Query openspeedtest deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service openspeedtest query -- [--instance a]
...
import {
  listOpenspeedtestDeploymentSummaries,
  normalizeOpenspeedtestConfig,
  resolveOpenspeedtestDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/openspeedtest-install.mjs";
import { queryOpenspeedtestInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeOpenspeedtestConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listOpenspeedtestDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveOpenspeedtestDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryOpenspeedtestInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.openspeedtest,
