 ...
 */
export async function installOpenspeedtestInCt(user, pveHost, vmid, openspeedtest, install) {
  errout.write(`[hdc] openspeedtest install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "openspeedtest install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const composeYaml = renderComposeYaml(openspeedtest, install);
  ...
  errout.write(`[hdc] openspeedtest install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(openspeedtest, ip),
    upstream_url: resolveUpstreamUrl(ip, openspeedtest),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainOpenspeedtestInCt(user, pveHost, vmid, openspeedtest, install, opts = {}) {
  errout.write(`[hdc] openspeedtest maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "openspeedtest maintain");
  if (!ready) {
