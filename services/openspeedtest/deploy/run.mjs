#!/usr/bin/env node
/**
 * Deploy openspeedtest on Proxmox LXC (Docker Compose, stateless).
 *
 * Usage: hdc run service openspeedtest deploy -- [--instance a | --system-id openspeedtest-a] [--skip-install]
 *        hdc run service openspeedtest deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveOpenspeedtestDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installOpenspeedtestInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/openspeedtest-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/openspeedtest-render.mjs";
import {
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/openspeedtest/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveOpenspeedtestDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, openspeedtest, install } = deployment;
  ...
  const openspeedtestCfg = isObject(openspeedtest) ? openspeedtest : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installOpenspeedtestInCt(pveSsh.user, pveSsh.host, guestVmid, openspeedtestCfg, installCfg);
  } else {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(openspeedtestCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, openspeedtestCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(openspeedtestCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: openspeedtest LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveOpenspeedtestDeployments>} */
  let deployments;
  try {
    deployments = resolveOpenspeedtestDeployments(cfg, flags);
  } catch (e) {
