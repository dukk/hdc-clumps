 ...
 */
export async function installN8nInCt(user, pveHost, vmid, n8n, install, encryptionKey) {
  errout.write(`[hdc] n8n install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "n8n install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const envContent = renderN8nEnv(n8n, encryptionKey, ip);
  ...
  errout.write(`[hdc] n8n install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(n8n, ip),
    upstream_url: resolveUpstreamUrl(ip, n8n),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainN8nInCt(user, pveHost, vmid, n8n, install, encryptionKey, opts = {}) {
  errout.write(`[hdc] n8n maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "n8n maintain");
  if (!ready) {
