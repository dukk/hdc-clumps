...
import { composeDir, hostPort, parsePublicUrl } from "./n8n-render.mjs";
 ...
 */
export async function queryN8nInCt(user, pveHost, vmid, n8n, install) {
  const cfg = isObject(n8n) ? n8n : {};
  ...
  if (docker.stdout.trim() === "active") {
    const healthCmd = publicUrl
      ? `curl -sf --max-time 5 ${JSON.stringify(`${publicUrl}/healthz`)} -o /dev/null && echo ok || echo fail`
      : `curl -sf --max-time 5 http://127.0.0.1:${port}/healthz -o /dev/null && echo ok || echo fail`;
