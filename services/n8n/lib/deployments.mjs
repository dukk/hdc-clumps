...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(N8N_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.n8n)) defaults.n8n = structuredClone(cfg.n8n);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeN8nConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("n8n config must be a JSON object");
  }
  ...
  throw new Error("n8n config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(N8N_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match n8n-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const n8n = isObject(d.n8n) ? d.n8n : {};
    const publicUrl = typeof n8n.public_url === "string" ? n8n.public_url.trim() : "";
    if (publicUrl) {
      parsePublicUrl(n8n);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listN8nDeploymentSummaries(cfg) {
  const { deployments } = normalizeN8nConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(n8n),
      host_port: hostPort(n8n),
      public_url: publicUrl,
      timezone: typeof n8n.timezone === "string" ? n8n.timezone : null,
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(N8N_ROLE).test(t)) return t;
  return lxcSystemId(N8N_ROLE, t);
}

/**
 * @param {Record<string, unknown>} n8n
 */
export function encryptionKeyVaultKeyFromConfig(n8n) {
  return encryptionKeyVaultKey(isObject(n8n) ? n8n : {});
}
 ...
 */
export function resolveN8nDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeN8nConfig(cfg);
