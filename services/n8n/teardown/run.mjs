#!/usr/bin/env node
/**
 * Teardown n8n Proxmox LXC deployments.
 *
 * Usage: hdc run service n8n teardown -- [--instance a | --system-id n8n-a]
 *        hdc run service n8n teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/n8n-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down n8n LXC (stderr log; JSON on stdout).\n`);
