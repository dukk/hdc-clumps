#!/usr/bin/env node
/**
 * Deploy n8n on Proxmox LXC (Docker Compose, SQLite).
 *
 * Usage: hdc run service n8n deploy -- [--instance a | --system-id n8n-a] [--skip-install]
 *        hdc run service n8n deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveN8nDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installN8nInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/n8n-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/n8n-render.mjs";
import {
...
import { createN8nVaultAccess } from "hdc/package/n8n-vault-deps.mjs";
import { resolveN8nEncryptionKey } from "hdc/package/vault-secrets.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/n8n/config.example.json";
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; encryptionKey: string }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, n8n, install } = deployment;
  ...
  if (shouldInstall(install)) {
    installResult = await installN8nInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      n8nCfg,
      installCfg,
      runOpts.encryptionKey,
    );
  } else {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(n8nCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, n8nCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(n8nCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: n8n LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveN8nDeployments>} */
  let deployments;
  try {
  ...
  for (const deployment of deployments) {
    try {
      results.push(await deployOne(deployment, flags, log, { ctPasswordCache, encryptionKey }));
    } catch (e) {
      const msg = String(/** @type {Error} */ (e).message || e);
