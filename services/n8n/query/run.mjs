#!/usr/bin/env node
/**
 * Query n8n deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service n8n query -- [--instance a]
...
import { resolvePveSshForHost } from "hdc/package/n8n-install.mjs";
import { queryN8nInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeN8nConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listN8nDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveN8nDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryN8nInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.n8n,
