#!/usr/bin/env node
/**
 * Deploy WireGuard hub on Proxmox LXC.
 *
 * Usage: hdc run service wireguard deploy -- [--instance a | --system-id wireguard-a] [--skip-install]
 *        hdc run service wireguard deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveWireguardDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installWireguardInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/wireguard-install.mjs";
import { interfaceAddress, listenPort, normalizePeers, privateKeyVaultKey, renderWg0Conf } from "hdc/package/wireguard-render.mjs";
...
import { createWireguardVaultAccess } from "hdc/package/vault-deps.mjs";

const here = dirname(fileURLToPath(import.meta.url));
const target = basename(dirname(here));
const verb = basename(here);
const clumpRoot = join(here, "..");
const CLUMP_CONFIG_EXAMPLE = "clumps/services/wireguard/config.example.json";
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; vault: ReturnType<typeof createWireguardVaultAccess> }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, wireguard, install } = deployment;
  ...
  if (!skipProvision) {
    ...
    const parameters = { ...lxc, unprivileged: 0, password: rootPassword };
  ...
  if (shouldInstall(install)) {
    const secrets = await loadWireguardSecrets(wireguardCfg, runOpts.vault);
    const wgConfig = renderWg0Conf(wireguardCfg, secrets);
    installResult = await installWireguardInCt(pveSsh.user, pveSsh.host, guestVmid, wgConfig);
  } else {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    interface_address: interfaceAddress(wireguardCfg),
    listen_port: listenPort(wireguardCfg),
    peers: normalizePeers(wireguardCfg).length,
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: WireGuard LXC via Proxmox (stderr log; JSON on stdout).\n`);
