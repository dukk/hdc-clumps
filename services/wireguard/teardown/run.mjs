#!/usr/bin/env node
/**
 * Teardown WireGuard Proxmox LXC deployments.
 *
 * Usage: hdc run service wireguard teardown -- [--instance a | --system-id wireguard-a]
 *        hdc run service wireguard teardown -- [--dry-run] [--yes]
 */
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down WireGuard LXC (stderr log; JSON on stdout).\n`);
