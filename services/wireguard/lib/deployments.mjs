...
import { interfaceAddress, listenPort, normalizePeers, privateKeyVaultKey } from "./wireguard-render.mjs";
...
function normalizeV1(cfg) {
  ...
  if (isObject(cfg.wireguard)) defaults.wireguard = structuredClone(cfg.wireguard);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeWireguardConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("wireguard config must be a JSON object");
  }
  ...
  throw new Error("wireguard config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match wireguard-<letter>`);
    }
    ...
    if (mode === "proxmox-lxc" || mode === "" || !mode) {
      ...
      if (lxc.unprivileged !== undefined && Number(lxc.unprivileged) !== 0) {
        throw new Error(`${sid}: proxmox.lxc.unprivileged must be 0 for WireGuard`);
      }
    }
    const wireguard = isObject(d.wireguard) ? d.wireguard : {};
    privateKeyVaultKey(wireguard);
    interfaceAddress(wireguard);
    listenPort(wireguard);
    const peers = normalizePeers(wireguard);
    for (const peer of peers) {
      if (!peer.public_key_vault_key) {
        throw new Error(`${sid}: wireguard peer ${JSON.stringify(peer.name)} missing public_key_vault_key`);
      }
      if (!peer.preshared_key_vault_key) {
        throw new Error(`${sid}: wireguard peer ${JSON.stringify(peer.name)} missing preshared_key_vault_key`);
      }
      if (!peer.allowed_ips.length) {
        throw new Error(`${sid}: wireguard peer ${JSON.stringify(peer.name)} requires allowed_ips[]`);
      }
    }
  }
}
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listWireguardDeploymentSummaries(cfg) {
  const { deployments } = normalizeWireguardConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      listen_port: listenPort(wireguard),
      interface_address: interfaceAddress(wireguard),
      peer_count: normalizePeers(wireguard).length,
      private_key_vault_key: privateKeyVaultKey(wireguard),
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(ROLE).test(t)) return t;
  return lxcSystemId(ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    wireguard: isObject(d.wireguard) ? d.wireguard : {},
    install,
  };
}
 ...
 */
export function resolveWireguardDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeWireguardConfig(cfg);
