#!/usr/bin/env node
/**
 * Query WireGuard deployments.
 *
 * Usage: hdc run service wireguard query -- [--instance a]
...
import {
  listWireguardDeploymentSummaries,
  normalizeWireguardConfig,
  resolveWireguardDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/wireguard-install.mjs";
import { queryWireguardInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeWireguardConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listWireguardDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveWireguardDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryWireguardInCt(pveSsh.user, pveSsh.host, vmid, d.wireguard);
