...
import { adminTokenVaultKey, hostPort, normalizeDomain, normalizeImageTag } from "./vaultwarden-render.mjs";

const VAULTWARDEN_ROLE = "vaultwarden";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(VAULTWARDEN_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.vaultwarden)) defaults.vaultwarden = structuredClone(cfg.vaultwarden);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeVaultwardenConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("vaultwarden config must be a JSON object");
  }
  ...
  throw new Error("vaultwarden config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(VAULTWARDEN_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match vaultwarden-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const vw = isObject(d.vaultwarden) ? d.vaultwarden : {};
    normalizeDomain(vw);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listVaultwardenDeploymentSummaries(cfg) {
  const { deployments } = normalizeVaultwardenConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(vw),
      host_port: hostPort(vw),
      domain,
      signups_allowed: vw.signups_allowed === true,
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(VAULTWARDEN_ROLE).test(t)) return t;
  return lxcSystemId(VAULTWARDEN_ROLE, t);
}

/**
 * @param {Record<string, unknown>} vaultwarden
 */
export function adminTokenVaultKeyFromConfig(vaultwarden) {
  return adminTokenVaultKey(isObject(vaultwarden) ? vaultwarden : {});
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    vaultwarden: isObject(d.vaultwarden) ? d.vaultwarden : {},
    install,
  };
}
 ...
 */
export function resolveVaultwardenDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeVaultwardenConfig(cfg);
