#!/usr/bin/env node
/**
 * Query Vaultwarden deployments (config summary + optional live CT status).
 *
 * Usage: hdc run vaultwarden query -- [--instance a]
...
import { resolvePveSshForHost } from "hdc/package/vaultwarden-install.mjs";
import { queryVaultwardenInCt } from "hdc/package/query-status.mjs";import { loadClumpConfigFromClumpRoot, tryLoadClumpConfigFromClumpRoot } from "hdc/package/clump-run-config.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeVaultwardenConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listVaultwardenDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveVaultwardenDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryVaultwardenInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.vaultwarden,
