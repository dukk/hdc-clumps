#!/usr/bin/env node
/**
 * Teardown Vaultwarden Proxmox LXC deployments.
 *
 * Usage: hdc run vaultwarden teardown -- [--instance a | --system-id vaultwarden-a]
 *        hdc run vaultwarden teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { resolveVaultwardenDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/vaultwarden-install.mjs";
import {
  confirmTeardown,
  teardownDryRun,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Vaultwarden LXC (stderr log; JSON on stdout).\n`);
