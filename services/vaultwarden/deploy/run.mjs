#!/usr/bin/env node
/**
 * Deploy Vaultwarden on Proxmox LXC (Docker Compose, nginx-waf HTTPS domain).
 *
 * Usage: hdc run vaultwarden deploy -- [--instance a | --system-id vaultwarden-a] [--skip-install]
 *        hdc run vaultwarden deploy -- [--skip-existing | --redeploy-existing]
 */
...
import {
  installVaultwardenInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/vaultwarden-install.mjs";
import {
  hostPort,
  resolveAdminUrl,
  resolveUpstreamUrl,
  resolveWebUrl,
} from "hdc/package/vaultwarden-render.mjs";
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; adminToken: string }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, vaultwarden, install } = deployment;
  ...
  /** @type {{ ok: boolean; method?: string; message?: string; web_url?: string; admin_url?: string; upstream_url?: string | null } | null} */
  ...
  if (shouldInstall(install)) {
    installResult = await installVaultwardenInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      vaultwardenCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    web_url: resolveWebUrl(vaultwardenCfg),
    admin_url: resolveAdminUrl(vaultwardenCfg),
    upstream_url: resolveUpstreamUrl(ip, vaultwardenCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(vaultwardenCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Vaultwarden LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveVaultwardenDeployments>} */
  let deployments;
  try {
  ...
  errout.write(`[hdc] ${target} ${verb}: admin token loaded from vault ${tokenKey}\n`);
  ...
  for (const deployment of deployments) {
    try {
      results.push(await deployOne(deployment, flags, log, { ctPasswordCache, adminToken }));
    } catch (e) {
      const msg = String(/** @type {Error} */ (e).message || e);
