#!/usr/bin/env node
/**
 * Query Scanopy deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service scanopy query -- [--instance a]
...
import { resolvePveSshForHost } from "hdc/package/scanopy-install.mjs";
import { queryScanopyInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeScanopyConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listScanopyDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveScanopyDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryScanopyInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.scanopy,
