#!/usr/bin/env node
/**
 * Deploy Scanopy on Proxmox LXC (Docker Compose).
 *
 * Usage: hdc run service scanopy deploy -- [--instance a | --system-id scanopy-a] [--skip-install]
 *        hdc run service scanopy deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveScanopyDeployments, postgresPasswordVaultKey } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import {
  installScanopyInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/scanopy-install.mjs";
import {
...
import { createScanopyVaultAccess } from "hdc/package/vault-deps.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/scanopy/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveScanopyDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null }; postgresPassword: string }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, scanopy, install } = deployment;
  ...
  if (mode === "aws-ecs") {
    ...
    const awsConfigPath = join(root, "clumps", "infrastructure", "aws");
    let awsBase = {};
    try {
      const loaded = tryLoadClumpConfigFromClumpRoot(awsConfigPath, {
        exampleRel: "clumps/infrastructure/aws/config.example.json",
      });
  ...
  if (shouldInstall(install)) {
    installResult = await installScanopyInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      scanopyCfg,
  ...
  const port =
    typeof scanopyCfg.port === "number" && Number.isFinite(scanopyCfg.port) ? scanopyCfg.port : 60072;
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Scanopy LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveScanopyDeployments>} */
  let deployments;
  try {
    deployments = resolveScanopyDeployments(cfg, flags);
  } catch (e) {
  ...
  const vault = createScanopyVaultAccess();
  const defaultsScanopy =
    isObject(cfg.defaults) && isObject(cfg.defaults.scanopy) ? cfg.defaults.scanopy : {};
  const pwKey = postgresPasswordVaultKey(defaultsScanopy);
  errout.write(`[hdc] ${target} ${verb}: loading Postgres password from vault ${pwKey} …\n`);
  await vault.unlock({});
  const postgresPassword = String(
    await vault.getSecret(pwKey, { promptLabel: `vault secret ${pwKey}` }),
  ).trim();
  if (!postgresPassword) {
    errout.write(`[hdc] ${target} ${verb}: Postgres password required — set vault ${pwKey}\n`);
    process.stdout.write(
  ...
  errout.write(`[hdc] ${target} ${verb}: Postgres password loaded from vault ${pwKey}\n`);
  ...
  for (const deployment of deployments) {
    try {
      results.push(await deployOne(deployment, flags, log, { ctPasswordCache, postgresPassword }));
    } catch (e) {
      const msg = String(/** @type {Error} */ (e).message || e);
