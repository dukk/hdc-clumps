#!/usr/bin/env node
/**
 * Teardown Scanopy Proxmox LXC deployments.
 *
 * Usage: hdc run service scanopy teardown -- [--instance a | --system-id scanopy-a]
 *        hdc run service scanopy teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { resolveScanopyDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/scanopy-install.mjs";
import {
  confirmTeardown,
  teardownDryRun,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Scanopy LXC (stderr log; JSON on stdout).\n`);
