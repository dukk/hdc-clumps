 ...
 *
 * Usage: hdc run service scanopy maintain -- [--instance a | --system-id scanopy-a] [--skip-clamav]
 */
...
import { maintainScanopyInCt, resolvePveSshForHost } from "hdc/package/scanopy-install.mjs";
...
async function maintainOne(deployment, flags, vaultAccess) {
  ...
  const result = await maintainScanopyInCt(pveSsh.user, pveSsh.host, vmid, installCfg);
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: refresh Scanopy Docker stack (stderr log; JSON on stdout).\n`);
