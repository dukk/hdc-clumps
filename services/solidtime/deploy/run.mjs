#!/usr/bin/env node
/**
 * Deploy SolidTime on Proxmox LXC.
 *
 * Usage: hdc run service solidtime deploy -- [--instance a | --system-id solidtime-a] [--skip-install]
 *        hdc run service solidtime deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveSolidtimeDeployments, normalizeVersionTag } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import {
  installSolidtimeInCt,
  readAppUrlFromEnv,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/solidtime-install.mjs";
...
import { createSolidtimeVaultAccess } from "hdc/package/vault-deps.mjs";
import { resolveSolidtimeDbPassword } from "hdc/package/vault-db-password.mjs";
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; dbPassword: string }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, solidtime, install } = deployment;
  ...
  /** @type {{ ok: boolean; method?: string; message?: string; version?: string } | null} */
  ...
  const version = normalizeVersionTag(
    typeof solidtimeCfg.version === "string" ? solidtimeCfg.version : "v0.12.2",
  );

  if (shouldInstall(install)) {
    installResult = await installSolidtimeInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      solidtimeCfg,
  ...
  const url = readAppUrlFromEnv(pveSsh.user, pveSsh.host, vmid) || (ip ? `http://${ip}/` : null);
  const ok = provisionResult.ok && installResult.ok;
  return {
    ok,
    system_id: systemId,
    host_id: hostId,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: SolidTime LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveSolidtimeDeployments>} */
  let deployments;
  try {
  ...
  const vault = createSolidtimeVaultAccess();
  errout.write(`[hdc] ${target} ${verb}: resolving SolidTime DB password (vault) …\n`);
  const defaultsSolidtime =
    isObject(cfg.defaults) && isObject(cfg.defaults.solidtime) ? cfg.defaults.solidtime : {};
  let dbPassword;
  try {
    dbPassword = await resolveSolidtimeDbPassword(vault, defaultsSolidtime);
  } catch (e) {
    const msg = String(/** @type {Error} */ (e).message || e);
    errout.write(`[hdc] ${target} ${verb}: vault error: ${msg}\n`);
  ...
  for (const deployment of deployments) {
    try {
      results.push(await deployOne(deployment, flags, log, { ctPasswordCache, dbPassword }));
    } catch (e) {
      const msg = String(/** @type {Error} */ (e).message || e);
