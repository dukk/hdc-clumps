...
const SOLIDTIME_ROLE = "solidtime";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(SOLIDTIME_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.solidtime)) defaults.solidtime = structuredClone(cfg.solidtime);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeSolidtimeConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("solidtime config must be a JSON object");
  }
  ...
  throw new Error("solidtime config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(SOLIDTIME_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match solidtime-<letter>`);
    }
...
export function normalizeVersionTag(tag) {
  ...
  if (!t) throw new Error("solidtime.version is required");
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listSolidtimeDeploymentSummaries(cfg) {
  const { deployments } = normalizeSolidtimeConfig(cfg);
  return deployments.map((d) => {
    ...
    const solidtime = isObject(d.solidtime) ? d.solidtime : {};
    const version =
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(SOLIDTIME_ROLE).test(t)) return t;
  return lxcSystemId(SOLIDTIME_ROLE, t);
}
...
export function instanceLetterFromSystemId(systemId) {
  const m = new RegExp("^solidtime-([a-z]+)$").exec(systemId.trim());
  return m ? m[1] : "";
}
 ...
 */
export function dbPasswordVaultKey(solidtime) {
  const key =
    typeof solidtime.db_password_vault_key === "string" && solidtime.db_password_vault_key.trim()
      ? solidtime.db_password_vault_key.trim()
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    solidtime: isObject(d.solidtime) ? d.solidtime : {},
    install,
  };
}
 ...
 */
export function resolveSolidtimeDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeSolidtimeConfig(cfg);
