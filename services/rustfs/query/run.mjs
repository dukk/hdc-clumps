#!/usr/bin/env node
/**
 * Query RustFS cluster deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service rustfs query -- [--instance a|b|c|d]
...
import {
  listRustfsDeploymentSummaries,
  normalizeRustfsConfig,
  resolveRustfsDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/rustfs-install.mjs";
import { queryRustfsInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeRustfsConfig(cfg);
      schemaVersion = norm.schemaVersion;
      clusterNodes = norm.deployments.length;
      deployments = listRustfsDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveRustfsDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryRustfsInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.rustfs,
