#!/usr/bin/env node
/**
 * Teardown RustFS Proxmox LXC cluster nodes.
 *
 * Usage: hdc run service rustfs teardown -- [--instance a|b|c|d]
 *        hdc run service rustfs teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { resolveRustfsDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/rustfs-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down RustFS LXC cluster (stderr log; JSON on stdout).\n`);
  ...
  // Destroy in reverse order (d → c → b → a) when tearing down full cluster
  const instance = flagGet(flags, "instance");
  const systemId = flagGet(flags, "system-id", "system_id");
  if (!instance && !systemId && deployments.length > 1) {
