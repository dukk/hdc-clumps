#!/usr/bin/env node
/**
 * Query Gatus deployment status.
 *
 * Usage: hdc run service gatus query -- [--instance a | --system-id gatus-a]
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeGatusConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listGatusDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveGatusDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (!configError && selected) {
      ...
      for (const d of selected) {
        ...
        try {
          ...
          const gatusCfg = isObject(d.gatus) ? d.gatus : {};
          const status = queryGatusStatusInCt(ssh.user, ssh.host, vmid, gatusCfg);
          const ip = readCtPrimaryIp(ssh.user, ssh.host, vmid);
          const version = readGatusVersionInCt(ssh.user, ssh.host, vmid);
  ...
  const payload = {
    ...
    deployments,
    live,
    live_results: live.length ? liveResults : undefined,
  };
