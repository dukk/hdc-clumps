#!/usr/bin/env node
/**
 * Deploy Gatus on Proxmox LXC.
 *
 * Usage: hdc run service gatus deploy -- [--instance a | --system-id gatus-a] [--skip-install]
 *        hdc run service gatus deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveGatusDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installGatusInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/gatus-install.mjs";
import { configureGatusInCt } from "hdc/package/gatus-configure.mjs";
import { gatusListenPort } from "hdc/package/gatus-render.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/gatus/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveGatusDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, gatus, install } = deployment;
  ...
  const gatusCfg = isObject(gatus) ? gatus : {};

  if (shouldInstall(install)) {
    installResult = await installGatusInCt(pveSsh.user, pveSsh.host, vmid, gatusCfg);
  } else {
  ...
  configureResult = configureGatusInCt(pveSsh.user, pveSsh.host, vmid, gatusCfg);

  const ip = readCtPrimaryIp(pveSsh.user, pveSsh.host, guestVmid);
  const port = gatusListenPort(gatusCfg);
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    dashboard_url: ip ? `http://${ip}:${port}` : null,
    result: provisionResult,
    install: installResult,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Gatus LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  let deployments;
  try {
    deployments = resolveGatusDeployments(cfg, flags);
  } catch (e) {
