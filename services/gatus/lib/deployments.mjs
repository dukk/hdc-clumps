...
import { normalizeGatusVersion } from "./gatus-install.mjs";

const GATUS_ROLE = "gatus";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(GATUS_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.gatus)) defaults.gatus = structuredClone(cfg.gatus);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeGatusConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("gatus config must be a JSON object");
  }
  ...
  throw new Error("gatus config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(GATUS_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match gatus-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);

    const gatus = isObject(d.gatus) ? d.gatus : {};
    normalizeGatusVersion(typeof gatus.version === "string" ? gatus.version : "v5.36.0");
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listGatusDeploymentSummaries(cfg) {
  const { deployments } = normalizeGatusConfig(cfg);
  return deployments.map((d) => {
    ...
    const gatus = isObject(d.gatus) ? d.gatus : {};
    const endpoints = Array.isArray(gatus.endpoints) ? gatus.endpoints : [];
    return {
      ...
      version: normalizeGatusVersion(typeof gatus.version === "string" ? gatus.version : "v5.36.0"),
      listen_port:
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(GATUS_ROLE).test(t)) return t;
  return lxcSystemId(GATUS_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    gatus: isObject(d.gatus) ? d.gatus : {},
    install,
  };
}
 ...
 */
export function resolveGatusDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeGatusConfig(cfg);
