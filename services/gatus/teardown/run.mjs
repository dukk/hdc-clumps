#!/usr/bin/env node
/**
 * Teardown Gatus Proxmox LXC deployments.
 *
 * Usage: hdc run service gatus teardown -- [--instance a | --system-id gatus-a]
 *        hdc run service gatus teardown -- [--dry-run] [--yes]
 */
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Gatus LXC (stderr log; JSON on stdout).\n`);
