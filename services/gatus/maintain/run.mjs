 ...
 *
 * Usage: hdc run service gatus maintain -- [--instance a | --system-id gatus-a]
 *        hdc run service gatus maintain -- [--skip-upgrade] [--skip-clamav]
 */
...
import { resolvePveSshForHost } from "hdc/package/gatus-install.mjs";
import { maintainGatusInCt } from "hdc/package/gatus-configure.mjs";
...
async function maintainOne(deployment, flags, vaultAccess) {
  const { systemId, proxmox: px, gatus } = deployment;
  ...
  const result = await maintainGatusInCt(pveSsh.user, pveSsh.host, vmid, gatusCfg, { skipUpgrade });
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: re-apply Gatus config (stderr log; JSON on stdout).\n`);
