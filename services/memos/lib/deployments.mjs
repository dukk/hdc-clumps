...
import { hostPort, normalizeDriver, normalizeImageTag, parsePublicUrl } from "./memos-render.mjs";

const MEMOS_ROLE = "memos";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(MEMOS_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.memos)) defaults.memos = structuredClone(cfg.memos);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeMemosConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("memos config must be a JSON object");
  }
  ...
  throw new Error("memos config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(MEMOS_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match memos-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const memos = isObject(d.memos) ? d.memos : {};
    const publicUrl = memos.public_url;
    if (publicUrl !== null && publicUrl !== undefined) {
      const s = typeof publicUrl === "string" ? publicUrl.trim() : "";
      if (s) parsePublicUrl(memos);
    }
    const driver = normalizeDriver(memos);
    if (driver !== "sqlite") {
      throw new Error(`${sid}: memos.driver ${JSON.stringify(driver)} is not supported in v1 (use sqlite)`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listMemosDeploymentSummaries(cfg) {
  const { deployments } = normalizeMemosConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(memos),
      host_port: hostPort(memos),
      driver: normalizeDriver(memos),
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(MEMOS_ROLE).test(t)) return t;
  return lxcSystemId(MEMOS_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    memos: isObject(d.memos) ? d.memos : {},
    install,
  };
}
 ...
 */
export function resolveMemosDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeMemosConfig(cfg);
