#!/usr/bin/env node
/**
 * Teardown memos Proxmox LXC deployments.
 *
 * Usage: hdc run service memos teardown -- [--instance a | --system-id memos-a]
 *        hdc run service memos teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/memos-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down memos LXC (stderr log; JSON on stdout).\n`);
