#!/usr/bin/env node
/**
 * Query memos deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service memos query -- [--instance a]
 *        hdc run service memos query -- --live
 */
...
import {
  listMemosDeploymentSummaries,
  normalizeMemosConfig,
  resolveMemosDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/memos-install.mjs";
import { queryMemosInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeMemosConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listMemosDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveMemosDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryMemosInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.memos,
