#!/usr/bin/env node
/**
 * Deploy Memos on Proxmox LXC (Docker Compose, SQLite).
 *
 * Usage: hdc run service memos deploy -- [--instance a | --system-id memos-a] [--skip-install]
 *        hdc run service memos deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveMemosDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installMemosInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/memos-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/memos-render.mjs";
import {
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/memos/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveMemosDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, memos, install } = deployment;
  ...
  const memosCfg = isObject(memos) ? memos : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installMemosInCt(pveSsh.user, pveSsh.host, guestVmid, memosCfg, installCfg);
  } else {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(memosCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, memosCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(memosCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: memos LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveMemosDeployments>} */
  let deployments;
  try {
    deployments = resolveMemosDeployments(cfg, flags);
  } catch (e) {
