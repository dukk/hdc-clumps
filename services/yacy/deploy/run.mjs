#!/usr/bin/env node
/**
 * Deploy YaCy on Proxmox LXC (Docker Compose).
 *
 * Usage: hdc run service yacy deploy -- [--instance a | --system-id yacy-a] [--skip-install]
 *        hdc run service yacy deploy -- [--skip-existing | --redeploy-existing] [--skip-admin-password]
 */
...
import { adminPasswordVaultKey, httpPort, resolveYacyDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installYacyInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/yacy-install.mjs";
import {
...
import { createYacyVaultAccess } from "hdc/package/vault-deps.mjs";
 ...
 * @param {{ ctPasswordCache?: { value: string | null }; adminPassword: string | null; skipAdminPassword: boolean }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, yacy, install } = deployment;
  ...
  if (shouldInstall(install)) {
    installResult = await installYacyInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      yacyCfg,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: YaCy LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveYacyDeployments>} */
  let deployments;
  try {
  ...
  if (!skipAdminPassword) {
    const vault = createYacyVaultAccess();
    ...
    await vault.unlock({});
    adminPassword = String(await vault.getSecret(pwKey, { promptLabel: `vault secret ${pwKey}` })).trim();
    if (!adminPassword) {
    ...
    errout.write(`[hdc] ${target} ${verb}: admin password loaded from vault ${pwKey}\n`);
  } else {
    errout.write(`[hdc] ${target} ${verb}: --skip-admin-password — not setting YaCy admin password.\n`);
  }
