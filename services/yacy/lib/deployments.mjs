...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(YACY_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.yacy)) defaults.yacy = structuredClone(cfg.yacy);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeYacyConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("yacy config must be a JSON object");
  }
  ...
  throw new Error("yacy config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(YACY_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match yacy-<letter>`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listYacyDeploymentSummaries(cfg) {
  const { deployments } = normalizeYacyConfig(cfg);
  return deployments.map((d) => {
    ...
    const yacy = isObject(d.yacy) ? d.yacy : {};
    return {
      ...
      image_tag: typeof yacy.image_tag === "string" ? yacy.image_tag : "latest",
      http_port: httpPort(yacy),
      https_port: httpsPort(yacy),
      peer_name: typeof yacy.peer_name === "string" ? yacy.peer_name : null,
    };
  });
}
 ...
 */
export function httpPort(yacy) {
  const p = typeof yacy.http_port === "number" ? yacy.http_port : Number(yacy.http_port);
 ...
 * @param {Record<string, unknown>} yacy
 */
export function httpsPort(yacy) {
  const p = typeof yacy.https_port === "number" ? yacy.https_port : Number(yacy.https_port);
  if (Number.isFinite(p) && p >= 1 && p <= 65535) return Math.floor(p);
  return 8443;
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(YACY_ROLE).test(t)) return t;
  return lxcSystemId(YACY_ROLE, t);
}
 ...
 */
export function resolveYacyDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeYacyConfig(cfg);
