#!/usr/bin/env node
/**
 * Teardown YaCy Proxmox LXC deployments.
 *
 * Usage: hdc run service yacy teardown -- [--instance a | --system-id yacy-a]
 *        hdc run service yacy teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { resolveYacyDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/yacy-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down YaCy LXC (stderr log; JSON on stdout).\n`);
