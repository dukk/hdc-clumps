#!/usr/bin/env node
/**
 * Deploy stirling-pdf on Proxmox LXC (Docker Compose).
 *
 * Usage: hdc run service stirling-pdf deploy -- [--instance a | --system-id stirling-pdf-a] [--skip-install]
 *        hdc run service stirling-pdf deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveStirlingPdfDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import {
  installStirlingPdfInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/stirling-pdf-install.mjs";
import {
  ...
  resolveUpstreamUrl,
  resolveWebUrl,
} from "hdc/package/stirling-pdf-render.mjs";
import {
...
import { createStirlingPdfVaultAccess } from "hdc/package/vault-deps.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/stirling-pdf/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveStirlingPdfDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null }; vault: import("hdc/cli/lib/vault-access.mjs").VaultAccess }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, stirling_pdf, install } = deployment;
  ...
  /** @type {{ ok: boolean; method?: string; message?: string; url?: string | null; upstream_url?: string | null } | null} */
  ...
  const stirlingPdfCfg = isObject(stirling_pdf) ? stirling_pdf : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    let adminPassword = "";
    if (enableLogin(stirlingPdfCfg)) {
      const adminKey = adminPasswordVaultKey(stirlingPdfCfg);
      errout.write(`[hdc] ${target} ${verb}: loading vault ${adminKey} …\n`);
      adminPassword = String(
        await runOpts.vault.getSecret(adminKey, { promptLabel: `vault secret ${adminKey}` }),
      ).trim();
      if (!adminPassword) {
        return {
          ok: false,
          ...
          host_id: hostId,
          mode,
          message: `missing vault ${adminKey}`,
        };
      }
    }
    installResult = await installStirlingPdfInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      stirlingPdfCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(stirlingPdfCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, stirlingPdfCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(stirlingPdfCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: stirling-pdf LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  let deployments;
  try {
    deployments = resolveStirlingPdfDeployments(cfg, flags);
  } catch (e) {
  ...
  const vault = createStirlingPdfVaultAccess();
  await vault.unlock({});
