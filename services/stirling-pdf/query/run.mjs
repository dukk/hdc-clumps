#!/usr/bin/env node
/**
 * Query stirling-pdf deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service stirling-pdf query -- [--instance a]
...
import {
  listStirlingPdfDeploymentSummaries,
  normalizeStirlingPdfConfig,
  resolveStirlingPdfDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/stirling-pdf-install.mjs";
import { queryStirlingPdfInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeStirlingPdfConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listStirlingPdfDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveStirlingPdfDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryStirlingPdfInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.stirling_pdf,
