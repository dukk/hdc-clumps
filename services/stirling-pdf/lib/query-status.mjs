...
import { composeDir, hostPort, parsePublicUrl } from "./stirling-pdf-render.mjs";
 ...
 */
export async function queryStirlingPdfInCt(user, pveHost, vmid, stirlingPdf, install) {
  const cfg = isObject(stirlingPdf) ? stirlingPdf : {};
  ...
  if (docker.stdout.trim() === "active") {
    const healthCmd = `curl -sf --max-time 10 http://127.0.0.1:${port}/api/v1/info/status 2>/dev/null | grep -q UP && echo ok || echo fail`;
