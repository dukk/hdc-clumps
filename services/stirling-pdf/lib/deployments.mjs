...
import {
  ...
  normalizeTimezone,
  parsePublicUrl,
} from "./stirling-pdf-render.mjs";

const STIRLING_PDF_ROLE = "stirling-pdf";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(STIRLING_PDF_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.stirling_pdf)) defaults.stirling_pdf = structuredClone(cfg.stirling_pdf);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeStirlingPdfConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("stirling-pdf config must be a JSON object");
  }
  ...
  throw new Error("stirling-pdf config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(STIRLING_PDF_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match stirling-pdf-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const stirlingPdf = isObject(d.stirling_pdf) ? d.stirling_pdf : {};
    const publicUrl = stirlingPdf.public_url;
    if (publicUrl !== null && publicUrl !== undefined) {
      const s = typeof publicUrl === "string" ? publicUrl.trim() : "";
      if (s) parsePublicUrl(stirlingPdf);
    }
    if (enableLogin(stirlingPdf) && !adminPasswordVaultKey(stirlingPdf)) {
      throw new Error(`${sid}: stirling_pdf.security.admin_password_vault_key required when login enabled`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listStirlingPdfDeploymentSummaries(cfg) {
  const { deployments } = normalizeStirlingPdfConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image: normalizeImage(stirlingPdf),
      host_port: hostPort(stirlingPdf),
      public_url: publicUrl,
      timezone: normalizeTimezone(stirlingPdf),
      langs: normalizeLangs(stirlingPdf),
      login_enabled: enableLogin(stirlingPdf),
      admin_password_vault_key: adminPasswordVaultKey(stirlingPdf),
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(STIRLING_PDF_ROLE).test(t)) return t;
  return lxcSystemId(STIRLING_PDF_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    stirling_pdf: isObject(d.stirling_pdf) ? d.stirling_pdf : {},
    install,
  };
}
 ...
 */
export function resolveStirlingPdfDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeStirlingPdfConfig(cfg);
