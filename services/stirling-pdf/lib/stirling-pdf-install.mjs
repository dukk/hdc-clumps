...
export function buildInstallScript(composeDirPath, composeYaml, envContent) {
  ...
  return [
    ...
    `mkdir -p '${dir}' '${dir}/configs' '${dir}/tessdata' '${dir}/logs' '${dir}/pipeline'`,
 ...
 */
export async function installStirlingPdfInCt(user, pveHost, vmid, stirlingPdf, install, adminPassword) {
  errout.write(`[hdc] stirling-pdf install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "stirling-pdf install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const envContent = renderStirlingPdfEnv(stirlingPdf, adminPassword);
  const composeYaml = renderComposeYaml(stirlingPdf);
  ...
  errout.write(`[hdc] stirling-pdf install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(stirlingPdf, ip),
    upstream_url: resolveUpstreamUrl(ip, stirlingPdf),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainStirlingPdfInCt(user, pveHost, vmid, stirlingPdf, install, adminPassword, opts = {}) {
  errout.write(`[hdc] stirling-pdf maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "stirling-pdf maintain");
  if (!ready) {
