#!/usr/bin/env node
/**
 * Teardown stirling-pdf Proxmox LXC deployments.
 *
 * Usage: hdc run service stirling-pdf teardown -- [--instance a | --system-id stirling-pdf-a]
 *        hdc run service stirling-pdf teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/stirling-pdf-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down stirling-pdf LXC (stderr log; JSON on stdout).\n`);
