 ...
 */
export async function installUnleashInCt(user, pveHost, vmid, unleash, install, secrets) {
  errout.write(`[hdc] unleash install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "unleash install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const envContent = renderUnleashEnv(unleash, secrets);
  ...
  errout.write(`[hdc] unleash install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(unleash, ip),
    upstream_url: resolveUpstreamUrl(ip, unleash),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainUnleashInCt(user, pveHost, vmid, unleash, install, secrets, opts = {}) {
  errout.write(`[hdc] unleash maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "unleash maintain");
  if (!ready) {
