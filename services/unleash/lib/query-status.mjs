...
import { composeDir, hostPort, parsePublicUrl } from "./unleash-render.mjs";
 ...
 */
export async function queryUnleashInCt(user, pveHost, vmid, unleash, install) {
  ...
  if (docker.stdout.trim() === "active") {
    const healthCmd = publicUrl
      ? `curl -sf --max-time 5 ${JSON.stringify(`${publicUrl}/health`)} -o /dev/null && echo ok || echo fail`
      : `curl -sf --max-time 5 http://127.0.0.1:${port}/health -o /dev/null && echo ok || echo fail`;
