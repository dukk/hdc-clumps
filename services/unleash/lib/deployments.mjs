...
import {
  ...
  parsePublicUrl,
} from "./unleash-render.mjs";

const UNLEASH_ROLE = "unleash";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(UNLEASH_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.unleash)) defaults.unleash = structuredClone(cfg.unleash);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeUnleashConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("unleash config must be a JSON object");
  }
  ...
  throw new Error("unleash config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(UNLEASH_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match unleash-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const unleash = isObject(d.unleash) ? d.unleash : {};
    const publicUrl = typeof unleash.public_url === "string" ? unleash.public_url.trim() : "";
    if (publicUrl) {
      parsePublicUrl(unleash);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listUnleashDeploymentSummaries(cfg) {
  const { deployments } = normalizeUnleashConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(unleash),
      postgres_image_tag: normalizePostgresImageTag(unleash),
      host_port: hostPort(unleash),
      public_url: publicUrl,
      log_level: normalizeLogLevel(unleash),
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(UNLEASH_ROLE).test(t)) return t;
  return lxcSystemId(UNLEASH_ROLE, t);
}

/**
 * @param {Record<string, unknown>} unleash
 */
export function adminPasswordVaultKeyFromConfig(unleash) {
  return adminPasswordVaultKey(isObject(unleash) ? unleash : {});
}

/**
 * @param {Record<string, unknown>} unleash
 */
export function dbPasswordVaultKeyFromConfig(unleash) {
  return dbPasswordVaultKey(isObject(unleash) ? unleash : {});
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    unleash: isObject(d.unleash) ? d.unleash : {},
    install,
  };
}
 ...
 */
export function resolveUnleashDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeUnleashConfig(cfg);
