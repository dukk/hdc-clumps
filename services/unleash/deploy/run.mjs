#!/usr/bin/env node
/**
 * Deploy unleash on Proxmox LXC (Docker Compose + PostgreSQL).
 *
 * Usage: hdc run service unleash deploy -- [--instance a | --system-id unleash-a] [--skip-install]
 *        hdc run service unleash deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveUnleashDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installUnleashInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/unleash-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/unleash-render.mjs";
import {
...
import { createUnleashVaultAccess } from "hdc/package/unleash-vault-deps.mjs";
import { resolveUnleashSecrets } from "hdc/package/vault-secrets.mjs";
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/unleash/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveUnleashDeployments>[number]} deployment
 * @param {Record<string, string>} flags
...
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, unleash, install } = deployment;
  ...
  const unleashCfg = isObject(unleash) ? unleash : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installUnleashInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      unleashCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(unleashCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, unleashCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(unleashCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: unleash LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveUnleashDeployments>} */
  let deployments;
  try {
    deployments = resolveUnleashDeployments(cfg, flags);
  } catch (e) {
  ...
  const vault = createUnleashVaultAccess();
  const defaultsUnleash =
    isObject(cfg.defaults) && isObject(cfg.defaults.unleash) ? cfg.defaults.unleash : {};
  let secrets;
  try {
    secrets = await resolveUnleashSecrets(vault, defaultsUnleash);
  } catch (e) {
    const msg = String(/** @type {Error} */ (e).message || e);
