#!/usr/bin/env node
/**
 * Query unleash deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service unleash query -- [--instance a]
...
import {
  listUnleashDeploymentSummaries,
  normalizeUnleashConfig,
  resolveUnleashDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/unleash-install.mjs";
import { queryUnleashInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeUnleashConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listUnleashDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveUnleashDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryUnleashInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.unleash,
