#!/usr/bin/env node
/**
 * Teardown unleash Proxmox LXC deployments.
 *
 * Usage: hdc run service unleash teardown -- [--instance a | --system-id unleash-a]
 *        hdc run service unleash teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/unleash-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down unleash LXC (stderr log; JSON on stdout).\n`);
