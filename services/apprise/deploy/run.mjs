#!/usr/bin/env node
/**
 * Deploy Apprise API on Proxmox LXC (Docker Compose).
 *
 * Usage: hdc run service apprise deploy -- [--instance a | --system-id apprise-a] [--skip-install]
 *        hdc run service apprise deploy -- [--skip-existing | --redeploy-existing] [--skip-keys]
 */
...
import { loadMailRelayAppSettings } from "hdc/package/mail-relay-settings.mjs";
...
import { allocateNextVmid } from "../../bind/lib/proxmox-qemu-redeploy.mjs";

import { persistAllocatedVmid, resolveAppriseDeployments } from "hdc/package/deployments.mjs";
...
import {
  ...
  seedAppriseKeysInCt,
} from "hdc/package/apprise-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/apprise-render.mjs";
import {
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/apprise/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveAppriseDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, apprise, install } = deployment;
  ...
  let vmid = typeof lxc.vmid === "number" ? lxc.vmid : Number(lxc.vmid);
  let allocatedVmid = false;
  if (!Number.isFinite(vmid) || vmid <= 0) {
    errout.write(`[hdc] ${target} ${verb}: ${systemId}: allocating next free VMID …\n`);
    const resourcesBody = await pveJsonRequest(
      "GET",
      auth.host.apiBase,
    ...
    const resources = pveData(resourcesBody);
    vmid = allocateNextVmid(Array.isArray(resources) ? resources : [], 100);
    lxc.vmid = vmid;
    allocatedVmid = true;
    errout.write(`[hdc] ${target} ${verb}: ${systemId}: allocated vmid ${vmid}.\n`);
  }
  ...
  if (!skipProvision) {
    ...
    const parameters = { ...lxc, vmid, password: rootPassword };
    ...
    if (allocatedVmid) {
      try {
        persistAllocatedVmid(ensurePackageConfig().path, systemId, vmid);
        errout.write(
          `[hdc] ${target} ${verb}: ${systemId}: wrote vmid ${vmid} to config.json.\n`,
        );
      } catch (e) {
        errout.write(
          `[hdc] ${target} ${verb}: ${systemId}: warning: could not persist vmid: ${String(/** @type {Error} */ (e).message || e)}\n`,
        );
      }
    }
  } else {
  ...
  const appriseCfg = isObject(apprise) ? apprise : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installAppriseInCt(pveSsh.user, pveSsh.host, guestVmid, appriseCfg, installCfg);
  } else {
  ...
  let keysResult = null;
  if (shouldInstall(install) && flagGet(flags, "skip-keys") === undefined) {
    try {
      const relay = loadMailRelayAppSettings();
      keysResult = seedAppriseKeysInCt(pveSsh.user, pveSsh.host, guestVmid, appriseCfg, relay);
      if (!keysResult.ok) {
        return {
          ok: false,
          system_id: systemId,
  ...
  return {
    ...
    vmid: guestVmid,
    ip,
    url: resolveWebUrl(appriseCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, appriseCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(appriseCfg),
    result: provisionResult,
    install: installResult,
    keys: keysResult,
  };
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Apprise API LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveAppriseDeployments>} */
  let deployments;
  try {
    deployments = resolveAppriseDeployments(cfg, flags);
  } catch (e) {
