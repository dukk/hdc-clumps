#!/usr/bin/env node
/**
 * Query Apprise deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service apprise query -- [--instance a]
...
import {
  listAppriseDeploymentSummaries,
  normalizeAppriseConfig,
  resolveAppriseDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/apprise-install.mjs";
import { queryAppriseInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeAppriseConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listAppriseDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveAppriseDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryAppriseInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.apprise,
