...
import { hostPort, normalizeImageTag, parsePublicUrl } from "./apprise-render.mjs";

const APPRISE_ROLE = "apprise";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(APPRISE_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.apprise)) defaults.apprise = structuredClone(cfg.apprise);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeAppriseConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("apprise config must be a JSON object");
  }
  ...
  throw new Error("apprise config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(APPRISE_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match apprise-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const apprise = isObject(d.apprise) ? d.apprise : {};
    const publicUrl = apprise.public_url;
    if (publicUrl !== null && publicUrl !== undefined) {
      const s = typeof publicUrl === "string" ? publicUrl.trim() : "";
      if (s) parsePublicUrl(apprise);
    }
    ...
    if (mode === "proxmox-lxc" || mode === "" || !mode) {
      ...
      if (!Number.isFinite(vmid) || vmid < 0) {
        throw new Error(`${sid}: proxmox.lxc.vmid must be a number (>= 0; 0 allocates on deploy)`);
      }
    }
  }
}
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listAppriseDeploymentSummaries(cfg) {
  const { deployments } = normalizeAppriseConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(apprise),
      host_port: hostPort(apprise),
      public_url: publicUrl,
      timezone: typeof apprise.timezone === "string" ? apprise.timezone : null,
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(APPRISE_ROLE).test(t)) return t;
  return lxcSystemId(APPRISE_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    apprise: isObject(d.apprise) ? d.apprise : {},
    install,
  };
}
 ...
 */
export function resolveAppriseDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeAppriseConfig(cfg);
...
/**
 * Persist an allocated VMID into hdc-private (or public) clump config.json.
 *
 * @param {string} configPath
 * @param {string} systemId
 * @param {number} vmid
 */
export function persistAllocatedVmid(configPath, systemId, vmid) {
  const raw = readFileSync(configPath, "utf8");
  const cfg = JSON.parse(raw);
  if (!isObject(cfg) || !Array.isArray(cfg.deployments)) {
    throw new Error(`cannot persist vmid: ${configPath} has no deployments[]`);
  }
  const entry = cfg.deployments.find((d) => isObject(d) && d.system_id === systemId);
  if (!isObject(entry)) {
    throw new Error(`cannot persist vmid: no deployment ${JSON.stringify(systemId)}`);
  }
  if (!isObject(entry.proxmox)) entry.proxmox = {};
  if (!isObject(entry.proxmox.lxc)) entry.proxmox.lxc = {};
  entry.proxmox.lxc.vmid = vmid;
