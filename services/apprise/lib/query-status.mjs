...
import { composeDir, hostPort, parsePublicUrl } from "./apprise-render.mjs";
 ...
 */
export async function queryAppriseInCt(user, pveHost, vmid, apprise, install) {
  const cfg = isObject(apprise) ? apprise : {};
  ...
  if (docker.stdout.trim() === "active") {
    const healthCmd = `curl -sf --max-time 5 http://127.0.0.1:${port}/status -o /dev/null && echo ok || echo fail`;
