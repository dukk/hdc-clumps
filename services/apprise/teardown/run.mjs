#!/usr/bin/env node
/**
 * Teardown Apprise Proxmox LXC deployments.
 *
 * Usage: hdc run service apprise teardown -- [--instance a | --system-id apprise-a]
 *        hdc run service apprise teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/apprise-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Apprise LXC (stderr log; JSON on stdout).\n`);
