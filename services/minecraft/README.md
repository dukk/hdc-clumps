...
Heap defaults in the example: `-Xms2G -Xmx5G` (`minecraft.java_heap_min` / `java_heap`). Production guests with a small world should keep heap well below VM RAM (e.g. 8G on a 16 GiB VM) so the kernel can cache region files. systemd `ExecStart` also includes [Aikar G1GC flags](https://docs.papermc.io/paper/aikars-flags) by default; override with `minecraft.java_jvm_args` (non-empty string replaces the default flag set). Grow the QEMU root disk on maintain when `defaults.proxmox.qemu.rootfs_gb` exceeds live size. Set `defaults.proxmox.qemu.cpu` to **`host`** (not the QEMU default `kvm64`) so the guest sees AVX/AES; maintain stop/starts the VM when CPU type changes (`--no-reboot` writes config only). Paper 1.21+ already bundles [spark](https://spark.lucko.me/); `minecraft.spark` records that we rely on it (no extra plugin jar — Modrinth only publishes Fabric/Forge). `minecraft.clamav_profile` (`lean` / `standard` / `full`) overrides the memory-based ClamAV profile (`lean` masks `clamd`).
...
**Plugin configs:** `query --import --yes` copies config-like files under guest `/opt/minecraft/plugins/` into hdc-private `plugin-configs/<rel>` as **native-format** files (same `.yml`/`.conf`/… content, not JSON wrappers). Log files are imported for archival but **never** applied on maintain. Skips userdata/cache/maps/`.archive-unpack`/translations, jars, DB files, backups, and secrets (e.g. `*.pem`). Maintain builds a change plan while the server is up, stops only when files differ, writes only those files, then starts. Plugin-config apply runs **after** guest baseline so a QEMU CPU/RAM stop-start cannot let plugins (BlueMap) rewrite sidecars from jar defaults. Use `--skip-plugin-configs` to leave guest plugin trees untouched. Optional `minecraft.plugin_configs.dir` (default `plugin-configs`).
