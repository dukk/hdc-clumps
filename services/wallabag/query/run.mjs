...
import {
  listWallabagDeploymentSummaries,
  normalizeWallabagConfig,
  resolveWallabagDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/wallabag-install.mjs";
import { queryWallabagInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeWallabagConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listWallabagDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveWallabagDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryWallabagInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.wallabag,
