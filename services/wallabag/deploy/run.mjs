#!/usr/bin/env node
/**
 * Deploy shlink on Proxmox LXC (Docker Compose + MariaDB + Redis).
 *
...
import { installWallabagInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/wallabag-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/wallabag-render.mjs";
import {
...
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, wallabag, install } = deployment;
  ...
  if (shouldInstall(install)) {
    installResult = await installWallabagInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      wallabagCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(wallabagCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, wallabagCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(wallabagCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: wallabag LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveWallabagDeployments>} */
  let deployments;
  try {
