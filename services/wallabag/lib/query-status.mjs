...
import { composeDir, hostPort, parsePublicUrl } from "./wallabag-render.mjs";
 ...
 */
export async function queryWallabagInCt(user, pveHost, vmid, wallabag, install) {
  ...
  if (docker.stdout.trim() === "active") {
    const healthCmd = `curl -sf --max-time 8 -o /dev/null -w '%{http_code}' http://127.0.0.1:${port}/ || echo fail`;
  ...
  return {
    ...
    url: publicUrl || (ctIp ? (port === 80 ? `http://${ctIp}` : `http://${ctIp}:${port}`) : null),
  };
}
