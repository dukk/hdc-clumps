 ...
 */
export async function installWallabagInCt(user, pveHost, vmid, wallabag, install, secrets) {
  errout.write(`[hdc] wallabag install: Docker Compose in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "wallabag install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const ip = readCtPrimaryIp(user, pveHost, vmid);
  const envContent = renderWallabagEnv(wallabag, secrets, ip);
  const composeYaml = renderComposeYaml(wallabag);
  ...
  errout.write(`[hdc] wallabag install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
    url: resolveWebUrl(wallabag, ip),
    upstream_url: resolveUpstreamUrl(ip, wallabag),
    ct_ip: ip,
  };
}
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainWallabagInCt(user, pveHost, vmid, wallabag, install, secrets, opts = {}) {
  errout.write(`[hdc] wallabag maintain: refreshing stack in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "wallabag maintain");
  if (!ready) {
