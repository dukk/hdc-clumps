...
import {
  dbPasswordVaultKey,
  hostPort,
  normalizePostgresImage,
  normalizeRedisImage,
  normalizeRevision,
  parsePublicUrl,
} from "./affine-render.mjs";

const AFFINE_ROLE = "affine";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(AFFINE_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.affine)) defaults.affine = structuredClone(cfg.affine);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeAffineConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("affine config must be a JSON object");
  }
  ...
  throw new Error("affine config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(AFFINE_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match affine-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const affine = isObject(d.affine) ? d.affine : {};
    const publicUrl = typeof affine.public_url === "string" ? affine.public_url.trim() : "";
    if (publicUrl) {
      parsePublicUrl(affine);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listAffineDeploymentSummaries(cfg) {
  const { deployments } = normalizeAffineConfig(cfg);
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(AFFINE_ROLE).test(t)) return t;
  return lxcSystemId(AFFINE_ROLE, t);
}

/**
 * @param {Record<string, unknown>} affine
 */
export function dbPasswordVaultKeyFromConfig(affine) {
  return dbPasswordVaultKey(isObject(affine) ? affine : {});
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    affine: isObject(d.affine) ? d.affine : {},
    install,
  };
}
 ...
 */
export function resolveAffineDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeAffineConfig(cfg);
