#!/usr/bin/env node
/**
 * Teardown AFFiNE Proxmox LXC deployments.
 *
 * Usage: hdc run service affine teardown -- [--instance a | --system-id affine-a]
 *        hdc run service affine teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/affine-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down AFFiNE LXC (stderr log; JSON on stdout).\n`);
