#!/usr/bin/env node
/**
 * Deploy AFFiNE on Proxmox LXC (Docker Compose + Postgres + Redis).
 *
 * Usage: hdc run service affine deploy -- [--instance a | --system-id affine-a] [--skip-install]
 *        hdc run service affine deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveAffineDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installAffineInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/affine-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/affine-render.mjs";
import {
...
import { createAffineVaultAccess } from "hdc/package/affine-vault-deps.mjs";
import { resolveAffineSecrets } from "hdc/package/vault-secrets.mjs";
...
/**
 * @param {ReturnType<typeof resolveAffineDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null }; secrets: { dbPassword: string } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, affine, install } = deployment;
  ...
  const affineCfg = isObject(affine) ? affine : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installAffineInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      affineCfg,
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(affineCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, affineCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(affineCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: AFFiNE LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveAffineDeployments>} */
  let deployments;
  try {
    deployments = resolveAffineDeployments(cfg, flags);
  } catch (e) {
  ...
  const vault = createAffineVaultAccess();
  const defaultsAffine =
    isObject(cfg.defaults) && isObject(cfg.defaults.affine) ? cfg.defaults.affine : {};
  let secrets;
  try {
    secrets = await resolveAffineSecrets(vault, defaultsAffine);
  } catch (e) {
    const msg = String(/** @type {Error} */ (e).message || e);
