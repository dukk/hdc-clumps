#!/usr/bin/env node
/**
 * Deploy Nextcloud AIO on Proxmox LXC (Docker Compose mastercontainer).
 *
 * Usage: hdc run nextcloud deploy -- [--instance a | --system-id nextcloud-a] [--skip-install]
 *        hdc run nextcloud deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveNextcloudDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import {
  installNextcloudInCt,
  readCtPrimaryIp,
  resolvePveSshForHost,
} from "hdc/package/nextcloud-install.mjs";
import { resolveAioInterfaceUrl } from "hdc/package/nextcloud-render.mjs";
...
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, nextcloud, install } = deployment;
  ...
  /** @type {{ ok: boolean; method?: string; message?: string; aio_interface_url?: string | null } | null} */
  ...
  if (shouldInstall(install)) {
    installResult = await installNextcloudInCt(
      pveSsh.user,
      pveSsh.host,
      guestVmid,
      nextcloudCfg,
  ...
  const aioUrl = resolveAioInterfaceUrl(ip, nextcloudCfg) ?? installResult.aio_interface_url ?? null;
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Nextcloud AIO LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveNextcloudDeployments>} */
  let deployments;
  try {
