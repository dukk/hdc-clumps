#!/usr/bin/env node
/**
 * Teardown Nextcloud AIO Proxmox LXC deployments.
 *
 * Usage: hdc run nextcloud teardown -- [--instance a | --system-id nextcloud-a]
 *        hdc run nextcloud teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/nextcloud-install.mjs";
import {
  confirmTeardown,
  teardownDryRun,
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Nextcloud AIO LXC (stderr log; JSON on stdout).\n`);
