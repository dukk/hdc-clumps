 ...
 */
export async function installNextcloudInCt(user, pveHost, vmid, nextcloud, install) {
  errout.write(`[hdc] nextcloud install: AIO mastercontainer in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "nextcloud install");
  if (!ready) {
    return { ok: false, method: "docker-compose", message: `CT ${vmid} not reachable via pct exec` };
  }

  const composeYaml = renderComposeYaml(nextcloud);
  ...
  const aioUrl = resolveAioInterfaceUrl(ip, nextcloud);
  errout.write(`[hdc] nextcloud install: completed on CT ${vmid}.\n`);
  return {
    ok: true,
    method: "docker-compose",
    message: "installed",
 ...
 * @param {{ skipUpgrade?: boolean }} [opts]
 */
export async function maintainNextcloudInCt(user, pveHost, vmid, nextcloud, install, opts = {}) {
  errout.write(`[hdc] nextcloud maintain: refreshing AIO mastercontainer in CT ${vmid} …\n`);

  const ready = await waitForCt(user, pveHost, vmid, 2000, "nextcloud maintain");
  if (!ready) {
  ...
  return {
    ok: true,
    message: opts.skipUpgrade ? "restarted" : "mastercontainer image refreshed",
    aio_interface_url: resolveAioInterfaceUrl(ip, nextcloud),
  };
}
