...
import { normalizeAioBlock } from "./nextcloud-render.mjs";

const NEXTCLOUD_ROLE = "nextcloud";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(NEXTCLOUD_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.nextcloud)) defaults.nextcloud = structuredClone(cfg.nextcloud);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeNextcloudConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("nextcloud config must be a JSON object");
  }
  ...
  throw new Error("nextcloud config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(NEXTCLOUD_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match nextcloud-<letter>`);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listNextcloudDeploymentSummaries(cfg) {
  const { deployments } = normalizeNextcloudConfig(cfg);
  return deployments.map((d) => {
    ...
    const nextcloud = isObject(d.nextcloud) ? d.nextcloud : {};
    const aio = normalizeAioBlock(nextcloud);
    return {
      ...
      aio_image_channel: aio.imageTag,
      interface_host_port: aio.interfaceHostPort,
      reverse_proxy: aio.reverseProxyEnabled,
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(NEXTCLOUD_ROLE).test(t)) return t;
  return lxcSystemId(NEXTCLOUD_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    nextcloud: isObject(d.nextcloud) ? d.nextcloud : {},
    install,
  };
}
 ...
 */
export function resolveNextcloudDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeNextcloudConfig(cfg);
