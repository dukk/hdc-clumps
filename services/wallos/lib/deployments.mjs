...
import { hostPort, normalizeImageTag, parsePublicUrl } from "./wallos-render.mjs";

const WALLOS_ROLE = "wallos";
...
function normalizeV1(cfg) {
  ...
  const systemId =
    typeof deploy.system_id === "string" && deploy.system_id.trim()
      ...
      : lxcSystemId(WALLOS_ROLE, "a");
  /** @type {Record<string, unknown>} */
  const defaults = { mode };
  if (isObject(cfg.proxmox)) defaults.proxmox = structuredClone(cfg.proxmox);
  if (isObject(cfg.wallos)) defaults.wallos = structuredClone(cfg.wallos);
 ...
 * @param {Record<string, unknown>} cfg
 */
export function normalizeWallosConfig(cfg) {
  if (!isObject(cfg)) {
    throw new Error("wallos config must be a JSON object");
  }
  ...
  throw new Error("wallos config needs deployments[] or legacy deploy + proxmox blocks");
}
...
function validateDeployments(deployments) {
  ...
  for (const d of deployments) {
    ...
    if (!deploymentSystemIdPattern(WALLOS_ROLE).test(sid)) {
      throw new Error(`system_id ${JSON.stringify(sid)} must match wallos-<letter>`);
    }
    if (ids.has(sid)) throw new Error(`duplicate system_id ${JSON.stringify(sid)}`);
    ids.add(sid);
    const wallos = isObject(d.wallos) ? d.wallos : {};
    const publicUrl = wallos.public_url;
    if (publicUrl !== null && publicUrl !== undefined) {
      const s = typeof publicUrl === "string" ? publicUrl.trim() : "";
      if (s) parsePublicUrl(wallos);
    }
 ...
 * @param {Record<string, unknown>} cfg
 */
export function listWallosDeploymentSummaries(cfg) {
  const { deployments } = normalizeWallosConfig(cfg);
  return deployments.map((d) => {
    ...
    return {
      ...
      image_tag: normalizeImageTag(wallos),
      host_port: hostPort(wallos),
      public_url: publicUrl,
      timezone: typeof wallos.timezone === "string" ? wallos.timezone : null,
    };
  });
}
...
export function instanceFlagToSystemId(instance) {
  ...
  if (deploymentSystemIdPattern(WALLOS_ROLE).test(t)) return t;
  return lxcSystemId(WALLOS_ROLE, t);
}
...
function finalizeDeployment(d, skipInstallCli, skipInstallOpt) {
  ...
  return {
    ...
    wallos: isObject(d.wallos) ? d.wallos : {},
    install,
  };
}
 ...
 */
export function resolveWallosDeployments(cfg, flags, opts = {}) {
  const { deployments } = normalizeWallosConfig(cfg);
