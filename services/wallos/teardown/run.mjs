#!/usr/bin/env node
/**
 * Teardown Wallos Proxmox LXC deployments.
 *
 * Usage: hdc run service wallos teardown -- [--instance a | --system-id wallos-a]
 *        hdc run service wallos teardown -- [--dry-run] [--yes] [--skip-compose-down]
 */
...
import { composeDownInCt, resolvePveSshForHost } from "hdc/package/wallos-install.mjs";
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: tear down Wallos LXC (stderr log; JSON on stdout).\n`);
