#!/usr/bin/env node
/**
 * Query Wallos deployments (config summary + optional live CT status).
 *
 * Usage: hdc run service wallos query -- [--instance a]
 *        hdc run service wallos query -- --live
 */
...
import {
  listWallosDeploymentSummaries,
  normalizeWallosConfig,
  resolveWallosDeployments,
} from "hdc/package/deployments.mjs";
import { resolvePveSshForHost } from "hdc/package/wallos-install.mjs";
import { queryWallosInCt } from "hdc/package/query-status.mjs";
...
async function main() {
  ...
  if (cfg) {
    try {
      const norm = normalizeWallosConfig(cfg);
      schemaVersion = norm.schemaVersion;
      deployments = listWallosDeploymentSummaries(cfg);
    } catch (e) {
  ...
  if (live && cfg && !configError) {
    let selected;
    try {
      selected = resolveWallosDeployments(cfg, flags);
    } catch (e) {
      configError = String(/** @type {Error} */ (e).message || e);
    }
    if (selected) {
      for (const d of selected) {
        ...
        try {
          ...
          const status = await queryWallosInCt(
            pveSsh.user,
            pveSsh.host,
            vmid,
            d.wallos,
