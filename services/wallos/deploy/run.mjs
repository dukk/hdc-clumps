#!/usr/bin/env node
/**
 * Deploy Wallos on Proxmox LXC (Docker Compose, SQLite).
 *
 * Usage: hdc run service wallos deploy -- [--instance a | --system-id wallos-a] [--skip-install]
 *        hdc run service wallos deploy -- [--skip-existing | --redeploy-existing]
 */
...
import { resolveWallosDeployments } from "hdc/package/deployments.mjs";
import { findClusterGuest } from "hdc/package/guest-exists.mjs";
import { installWallosInCt, readCtPrimaryIp, resolvePveSshForHost } from "hdc/package/wallos-install.mjs";
import { hostPort, resolveUpstreamUrl, resolveWebUrl } from "hdc/package/wallos-render.mjs";
import {
...
const CLUMP_CONFIG_EXAMPLE = "clumps/services/wallos/config.example.json";
...
/**
 * @param {ReturnType<typeof resolveWallosDeployments>[number]} deployment
 * @param {Record<string, string>} flags
 * @param {import("../../../lib/host-provisioner.mjs").ProvisionLog} log
 * @param {{ ctPasswordCache?: { value: string | null } }} runOpts
 */
async function deployOne(deployment, flags, log, runOpts) {
  const { mode, systemId, proxmox: px, wallos, install } = deployment;
  ...
  const wallosCfg = isObject(wallos) ? wallos : {};
  const installCfg = isObject(install) ? install : {};

  if (shouldInstall(install)) {
    installResult = await installWallosInCt(pveSsh.user, pveSsh.host, guestVmid, wallosCfg, installCfg);
  } else {
  ...
  return {
    ...
    redeploy: skipProvision,
    ip,
    url: resolveWebUrl(wallosCfg, ip),
    upstream_url: resolveUpstreamUrl(ip, wallosCfg) ?? installResult.upstream_url ?? null,
    host_port: hostPort(wallosCfg),
    result: provisionResult,
    install: installResult,
  };
}
...
async function main() {
  errout.write(`[hdc] ${target} ${verb}: Wallos LXC via Proxmox (stderr log; JSON on stdout).\n`);
  ...
  /** @type {ReturnType<typeof resolveWallosDeployments>} */
  let deployments;
  try {
    deployments = resolveWallosDeployments(cfg, flags);
  } catch (e) {
