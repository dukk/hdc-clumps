 ...
 * } | null}
 */
export function deploymentStartupRow(deployment, defaults, clumpId, proxmoxCfg) {
...
export function collectStartupTargetsFromPackages(root, proxmoxCfg) {
  /** @type {Map<string, ReturnType<typeof deploymentStartupRow> & { clumpId: string }>} */
...
export async function runProxmoxGuestStartupMaintain(opts) {
  ...
  const loaded = loadProxmoxMaintainConfig(clumpRoot, warn, "Guest startup maintain");
  if (!loaded) {
  ...
  if (!targets.length) {
    warn("guest startup maintain: no startup targets found in service clump configs — skip.");
    return { ok: true, skipped: false, results: [] };
  }
